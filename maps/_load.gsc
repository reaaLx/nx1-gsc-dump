...
main( custom_hud_load )
{
	if ( IsDefined( custom_hud_load ) )
	{
		for ( index = 0; index < custom_hud_load.size; index++ )
		{
			switch ( custom_hud_load[index] )
			{
				case "hud_chute":
					SetSavedDvar( "hud_chute_load", 1 );
					break;

				case "hud_lunar":
					SetSavedDvar( "hud_lunar_load", 1 );
					break;

				case "hud_railgun":
					SetSavedDvar( "hud_railgun_load", 1 );
					break;

				default:
					//Assert( "You must specify a valid value for 'custom_hud_load' in _load::main()" );
					break;
			}
		}
	}
	...
	level._func[ "setsaveddvar" ] 	 = ::setsaveddvar;
	level._func[ "useanimtree" ] 	 = ::useAnimTree;
	level._func[ "setanim" ]			 = ::setAnim;
	level._func[ "setanimknob" ]	 	 = ::setAnimKnob;
	level._func[ "clearanim" ]		 = ::clearAnim;
	level._func[ "kill" ]			 = ::Kill;
	...
	level._global_callbacks = [];

	level._global_callbacks[ "_autosave_stealthcheck" ] 		 = ::global_empty_callback;
	level._global_callbacks[ "_patrol_endon_spotted_flag" ] 	 = ::global_empty_callback;
	level._global_callbacks[ "_spawner_stealth_default" ] 	 = ::global_empty_callback;
	//this one's for _idle...its peppered with stealth stuff so it got quarentined too.
	level._global_callbacks[ "_idle_call_idle_func" ] 		 = ::global_empty_callback;
	...
	if ( !isdefined( level._visionThermalDefault ) )
		level._visionThermalDefault = "cheat_bw";
	VisionSetThermal( level._visionThermalDefault );

	VisionSetPain( "near_death" );

	level._func[ "damagefeedback" ] = maps\_damagefeedback::updateDamageFeedback;// for player shooting sentry guns
	...
	level._first_frame = true;
	level._level_specific_dof = false;
	...
	*/
	...
	level._dirtEffectMenu[ "center" ] = "dirt_effect_center";
	level._dirtEffectMenu[ "left" ] = "dirt_effect_left";
	level._dirtEffectMenu[ "right" ] = "dirt_effect_right";
	PrecacheMenu( level._dirtEffectMenu[ "center" ] );
	PrecacheMenu( level._dirtEffectMenu[ "left" ] );
	PrecacheMenu( level._dirtEffectMenu[ "right" ] );
	PreCacheShader( "mtl_overlay_grit_bottom_b" );
	PreCacheShader( "mtl_overlay_grit_bottom" );
	PreCacheShader( "mtl_overlay_grit_left" );
	PreCacheShader( "mtl_overlay_grit_right" );

	// Melee system. 
	maps\_melee_vignettes::init_system();

	level._is_legacy_map = set_legacy_map();
	...
	else
	{
		...
		flags = GetArrayKeys( level._flag );
	...
	level._radiation_totalpercent = 0;
	...
	level._default_run_speed = 190;
	SetSavedDvar( "g_speed", level._default_run_speed );
	...
	level._arcadeMode_kill_func = ::arcadeMode_kill;
	level._connectPathsFunction = ::connectPaths;
	level._disconnectPathsFunction = ::disconnectPaths;
	level._badplace_cylinder_func = ::badplace_cylinder;
	level._badplace_delete_func = ::badplace_delete;
	level._isAIfunc = ::isAI;
	level._createClientFontString_func = maps\_hud_util::createClientFontString;
	level._HUDsetPoint_func = maps\_hud_util::setPoint;
	level._makeEntitySentient_func = ::makeEntitySentient;
	level._freeEntitySentient_func = ::freeEntitySentient;
	level._sentry_money_init_func = maps\_money::AI_money_init;
	level._laserOn_func = ::laserForceOn;
	level._laserOff_func = ::laserForceOff;
	level._stat_track_kill_func = maps\_player_stats::register_kill;
	level._stat_track_damage_func = maps\_player_stats::register_shot_hit;
	level._doPickyAutosaveChecks = true;
	level._autosave_threat_check_enabled = true;
	level._getNodeFunction = ::GetNode;
	level._getNodeArrayFunction = ::GetNodeArray;
	...
	{
		player.maxhealth = level._player.health;
	...
	level._last_mission_sound_time = -5000;
	level._hero_list = [];
...
// 	level.touched_eq_function[ false ] = ::ai_touched_eq_trigger;
	...
	common_scripts\_viewmodel_ui::viewmodel_ui_setup();
	...
	/#
	if ( string_starts_with( level._script, "so_" ) && !is_specialop() )
	...
	if ( !isdefined( level._trigger_flags ) )
	{
		// may have been defined by AI spawning
		init_trigger_flags();
	}

	level._killspawn_groups = [];
	...
	if ( level._start_point == "no_game" )
	{
		...
		if ( IsDefined( level._custom_no_game_setupFunc ) )
		{
			level [[ level._custom_no_game_setupFunc ]]();
		}
	...
	PrintLn( "level.script: ", level._script );
	...
	if ( !isdefined( level._MissionFailed ) )
		level._MissionFailed = false;
	...
	SetSavedDvar( "ui_campaign", level._campaign );// level.campaign is set in maps\_loadout::init_loadout
...
// 	thread usedAnimations();
	...
	array_thread( GetEntArray( "cover_traverse", "script_noteworthy" ), ::coverTraverseThink );
	...
	level._friendly_spawner[ "rifleguy" ] = GetEntArray( "rifle_spawner", "script_noteworthy" );
	level._friendly_spawner[ "smgguy" ] = GetEntArray( "smg_spawner", "script_noteworthy" );
	level._spawn_funcs = [];
	level._spawn_funcs[ "allies" ] = [];
	level._spawn_funcs[ "axis" ] = [];
	level._spawn_funcs[ "team3" ] = [];
	level._spawn_funcs[ "neutral" ] = [];
...
// 	array_thread( GetEntArray( "ally_spawn", "targetname" ), maps\_spawner::squadThink );
	...
	level._shared_portable_turrets = [];
	level._spawn_groups = [];
	maps\_spawner::main();

	// tagBK< NOTE > Moved this call until after the _spawner::main() so that ai group utilities could be used on 
	// actors that are spawned in vehicles. Calls like "waittill_aigroupcleared" were not previously working. 
...
// 	level.player thread stun_test();
	...
	assert( isdefined( level._missionsettings ) && isdefined( level._missionsettings.levels ) );
	assert( isdefined( level._script ) );

	if ( level._script == level._missionsettings.levels[0].name && !( level._player getLocalPlayerProfileData( "hasEverPlayed_SP" ) ) )
	{
		level._player SetLocalPlayerProfileData( "hasEverPlayed_SP", true );
		UpdateGamerProfile();
	}

	// SetSavedDvar( "cg_breathingSoundsEnabled", "1" );
	SetSavedDvar( "cg_minBreathingLevel", "3" );
	SetSavedDvar( "player_breathingLevel2Cutoff", "4000" );
	SetSavedDvar( "player_breathingLevel3Cutoff", "3500" );
	...
	// Default melee. 
	level._player maps\_melee_vignettes::activate_system( "melee_knife" );

	// secondary effects setup
	common_scripts\_nx_fx::fx_setup_secondary_damage_effects();
}
...
/*QUAKED trigger_damage_flag_set (0.12 0.23 1.0) ? PISTOL_NO RIFLE_NO PROJ_NO EXPLOSION_NO SPLASH_NO MELEE_NO
defaulttexture="flag"
Sets the script_flag when the trigger is damaged. */
...
/*QUAKED trigger_multiple_slideshoot (1.0 0.23 1.0) ?
defaulttexture="trigger"
Forces the player to slide but allows him to shoot.
*/
...
/*QUAKED script_struct_heli (0.9 0.8 0.0) (-15 -15 -15) (15 15 15) START_NODE SMOOTHABLE
*/
...
/*QUAKED trigger_audio (1 .3 .1) ? AI_AXIS AI_ALLIES AI_NEUTRAL NOTPLAYER VEHICLE TRIGGER_SPAWN TOUCH_ONCE
defaulttexture="audio"
wait- : -1 = one time only, otherwise triggers every frame.
default: "nx_verbType" "undefined"
Variable sized repeatable audio trigger.
*/
...
get_load_trigger_classes()
{
	...
	trigger_classes[ "trigger_multiple_slideshoot" ] = ::trigger_slideshoot;
	...
	trigger_classes[ "trigger_damage_flag_set" ] = ::trigger_damage_flag_set;
...
// 	trigger_funcs[ "eq_trigger" ] = ::eq_trigger;
	...
	trigger_funcs[ "trigger_slideshoot" ] = ::trigger_slideshoot;
...
{
	level._early_level = [];
	level._early_level[ "trainer" ] = true;
	level._early_level[ "roadkill" ] = true;
	level._early_level[ "cliffhanger" ] = true;
	level._early_level[ "airport" ] = true;
	level._early_level[ "favela" ] = true;
...
trigger_slideshoot( trigger )
{
	while ( 1 )
	{
		trigger waittill( "trigger", player );
		player thread slideshootTriggerPlayerThink( trigger );
	}
}
...
slideTriggerPlayerThink( trig )
{
	...
	if ( IsDefined( level._end_slide_delay ) )
		wait( level._end_slide_delay );
	self EndSliding();
}

slideshootTriggerPlayerThink( trig )
{
	if ( IsDefined( self.vehicle ) )
		return;
...
setup_simple_primary_lights()
{
	...
	burning_trash_lights = GetEntArray( "burning_trash_fire", "targetname" );
	tv_lights = GetEntArray( "television", "targetname" );
	...
	array_thread( burning_trash_lights, maps\_lights::burning_trash_fire );
	array_thread( tv_lights, maps\_lights::television );
}
...
usedAnimations()
{
	...
	while ( 1 )
	{
		...
		if ( !isdefined( level._completedAnims[ animname ] ) )
		{
			PrintLn( "^d -- -- No anims for ", animname, "^d -- -- -- -- -- - " );
			continue;
		}

		PrintLn( "^d -- -- Used animations for ", animname, "^d: ", level._completedAnims[ animname ].size, "^d -- -- -- -- -- - " );
		for ( i = 0; i < level._completedAnims[ animname ].size; i++ )
			PrintLn( level._completedAnims[ animname ][ i ] );
	}
}
...
{
	if ( !isdefined( level._badPlaces ) )
		level._badPlaces = 0;

	level._badPlaces++;
	BadPlace_Cylinder( "badplace" + level._badPlaces, -1, badplace.origin, badplace.radius, 1024 );
}

setup_individual_exploder( ent )
{
	exploder_num = ent.script_exploder;
	if ( !isdefined( level._exploders[ exploder_num ] ) )
	{
		level._exploders[ exploder_num ] = [];
	}
	...
	level._exploders[ exploder_num ][ level._exploders[ exploder_num ].size ] = ent;
...
nearAIRushesPlayer()
{
	if ( IsAlive( level._enemySeekingPlayer ) )
		return;
	enemy = get_closest_ai( level._player.origin, "bad_guys" );
	if ( !isdefined( enemy ) )
		return;

	if ( Distance( enemy.origin, level._player.origin ) > 400 )
		return;

	level._enemySeekingPlayer = enemy;
...
playerDamageShellshock()
{
	while ( true )
	{
		level._player waittill( "damage", damage, attacker, direction_vec, point, cause );
			...
			cause == "MOD_PROJECTILE_SPLASH" )
		{
			...
			multiplier = level._player.maxhealth / 100;
			...
			if ( time )
				level._player ShellShock( "default", time );
		}
	}
}
...
{
	/#
	if ( IsDefined( level._testmap ) )
		return true;
	#/

	if ( IsDefined( level._early_level[ level._script ] ) )
		return level._early_level[ level._script ];
	else
		return false;
}
...
player_special_death_hint()
{
	...
	level._player thread player_throwgrenade_timer();

	level._player waittill( "death", attacker, cause, weaponName );
	...
	{
		if ( ( level._player.lastgrenadetime - GetTime() ) > 3.5 * 1000 )
	...
	if ( cause == "MOD_EXPLOSIVE" )
	{
		if ( level._player destructible_death( attacker ) )
			return;


		if ( level._player exploding_barrel_death_af_chase( attacker ) )
			return;

		if ( level._player vehicle_death( attacker ) )
			return;

		if ( level._player exploding_barrel_death( attacker ) )
			return;
	}
...
exploding_barrel_death( attacker )
{
	...
	if ( IsDefined( level._lastExplodingBarrel ) )
	{
		// killed the same frame a barrel exploded
		if ( GetTime() != level._lastExplodingBarrel[ "time" ] )
			return false;

		// within the blast radius of the barrel that exploded
		d = Distance( self.origin, level._lastExplodingBarrel[ "origin" ] );
		if ( d > level._lastExplodingBarrel[ "radius" ] )
			return false;
...
{
	level._player.failingMission = true;
	...
	if ( IsDefined( textLine2 ) )
	{
		...
		fontElem.fontscale = 1.5;
		fontElem.x = 0;
		fontElem.y = -25 + level._fontHeight * fontElem.fontscale;
		fontElem.alignX = "center";
		fontElem.alignY = "middle";
		fontElem.horzAlign = "center";
		fontElem.vertAlign = "middle";
...
grenade_death_indicator_hudelement()
{
	...
	overlay.color = ( getdvarfloat("color_enemy_r"), getdvarfloat("color_enemy_g"), getdvarfloat("color_enemy_b") );
	overlay.alpha = 0;
...
triggered_playerseek( trig )
{
	...
	for ( i = 0; i < ai.size; i++ )
	{
		...
		{
			ai[ i ].goalradius = 800;
			ai[ i ] SetGoalEntity( level._player );
...
coverTraverseThink()
{
	cover_traverse_setup();

	// The trigger is passed in, the trigger's target is the cover node
	cover_node = GetNode( self.target, "targetname" );
	
	self thread cover_traverse_think_loop( cover_node );
}
...
cover_traverse_setup()
{
	//level._scr_animtree[ "cover_dude" ] = #animtree;
	level._scr_anim[ "cover_dude" ][ "tp_moon_trav_cover_crouch_R_L_128" ] = %tp_moon_trav_cover_crouch_R_L_128;
	level._scr_anim[ "cover_dude" ][ "tp_moon_trav_cover_crouch_L_R_128" ] = %tp_moon_trav_cover_crouch_L_R_128;

	level._cover_traverse_wait_min = 5.0;
	level._cover_traverse_wait_max = 8.0;
}
...
//                                                                  *
//*******************************************************************
cover_traverse_think_loop( node )
{
	// Wait until an AI triggers
	...
	while ( 1 )
	{
		if ( IsDefined( dude.covernode ) && dude.covernode == node )
		{
			// Wait a random amount of time
			wait RandomFloatRange( level._cover_traverse_wait_min, level._cover_traverse_wait_max );

			// The cover node's target is the orientation struct
			orientation = GetStruct( node.target, "targetname" );

			// The orientation struct's target is the other cover node
			other_node = GetNode( orientation.target, "targetname" );

			// Ensure we can do the move
			if ( dude IsTouching( self ) && IsDefined( dude.covernode ) && dude.covernode == node && dude MayMoveToPoint( other_node.origin ) )
			{
				dude SetGoalNode( other_node );

				// Play the animation (vignette style to break cover state)
				dude.animname = "cover_dude";
				dude.allowdeath = true;
				orientation maps\_anim::anim_single_solo( dude, orientation.animation );
			}
		}
...
bcTrigger( trigger )
{
	...
	if ( IsDefined( realTrigger ) )
	{
		...
		if ( ( other.team != level._player.team ) && level._player IsTouching( trigger ) )
		{
			soldier = level._player animscripts\battlechatter::getClosestFriendlySpeaker( "custom" );
		}
		// friendly touched auxiliary trigger, have the enemy AI chatter about it
		else if ( other.team == level._player.team )
		{
			enemyTeam = "axis";
			if ( level._player.team == "axis" )
			{
				enemyTeam = "allies";
			}
			...
			soldiers = get_array_of_farthest( level._player.origin, soldiers );
...
{
	if ( Distance( speakerOrigin, level._player GetOrigin() ) <= 512 )
	{
		return true;
	}
...
waterThink()
{
	...
	level._depth_allow_prone = 8;
	level._depth_allow_crouch = 33;
	level._depth_allow_stand = 50;
	...
	for ( ;; )
	{
		...
		while ( level._player IsTouching( self ) )
		{
			...
			if ( abs( d ) > level._depth_allow_crouch )
				level._player AllowCrouch( false );
			else
				level._player AllowCrouch( true );

			if ( abs( d ) > level._depth_allow_prone )
...
trigger_lookat_think( trigger, endOnFlag )
{
	...
	if ( has_flag )
	{
		...
		if ( !isdefined( level._flag[ flagName ] ) )
			flag_init( flagName );
	}
	else
	{
...
handle_starts()
{
	...
	if ( IsDefined( level._start_point ) )
		start = level._start_point;
	...
	for ( i = 0; i < dvars.size; i++ )
	{
		...
		{
			start_index = i;
			level._start_point = dvars[ i ];
			break;
		}
	}

	// then try based on the override
	if ( IsDefined( level._default_start_override ) && !isdefined( level._start_point ) )
	{
		foreach ( index, dvar in dvars )
		{
			if ( level._default_start_override == dvar )
			{
				start_index = index;
				level._start_point = dvar;
				break;
			}
		}
	}
	...
	if ( !isdefined( level._start_point ) )
	{
		...
		if ( level_has_start_points() )
			level._start_point = level._start_functions[ 0 ][ "name" ];
		else
			level._start_point = "default";
	}
	...
	if ( level._start_point == "default" )
	{
		if ( IsDefined( level._default_start ) )
		{
			level thread [[ level._default_start ]]();
		}
	...
	{
		/#
		if ( IsDefined( level._start_loc_string[ level._start_point ] ) )
			thread indicate_start( level._start_loc_string[ level._start_point ] );
		else
			thread indicate_start( level._start_point );
		#/
		start_array = level._start_arrays[ level._start_point ];
	...
	for ( i = start_index; i < level._start_functions.size; i++ )
	{
		start_array = level._start_functions[ i ];
...
start_menu()
{
	if ( !isdefined( level._start_loc_string ) )
	{
		 level._start_loc_string = [];
	}
	...
	for ( ;; )
	{
		...
		{
			level._display_starts_Pressed = false;
		}
	}
}
...
{
	dvars = [];
	for ( i = 0; i < level._start_functions.size; i++ )
	{
		dvars[ dvars.size ] = level._start_functions[ i ][ "name" ];
	}
	return dvars;
}
...
display_starts()
{
	...
	level._display_starts_Pressed = true;
	if ( level._start_functions.size <= 0 )
		return;
	...
	for ( i = 0; i < dvars.size; i++ )
	{
		...
		if ( IsDefined( level._start_loc_string[ dvar ] ) )
		{
			start_string += " -> ";
			start_string += level._start_loc_string[ dvar ];
		}
	...
	for ( ;; )
	{
		if ( !( level._player ButtonPressed( "F10" ) ) )
		{
			level._display_starts_Pressed = false;
		}
		...
		if ( !up_pressed )
		{
			if ( level._player ButtonPressed( "UPARROW" ) || level._player ButtonPressed( "DPAD_UP" ) || level._player ButtonPressed( "APAD_UP" ) )
			{
				up_pressed = true;
				selected++;
			}
		...
		if ( !down_pressed )
		{
			if ( level._player ButtonPressed( "DOWNARROW" ) || level._player ButtonPressed( "DPAD_DOWN" ) || level._player ButtonPressed( "APAD_DOWN" ) )
			{
				down_pressed = true;
				selected--;
			}
		...
		if ( level._player ButtonPressed( "kp_enter" ) || level._player ButtonPressed( "BUTTON_A" ) || level._player ButtonPressed( "enter" ) )
		{
			...
			level._player OpenPopupMenu( "start" );
...
start_button_combo()
{
	if ( !( level._player ButtonPressed( "BUTTON_RSTICK" ) ) )
		return false;

	if ( !( level._player ButtonPressed( "BUTTON_RSHLDR" ) ) )
		return false;
...
devhelp()
{
	...
	{
		update = false;
		if ( level._player ButtonPressed( "F1" ) )
		{
				devhelp_hudElements( hudarray, 1 );
				while ( level._player ButtonPressed( "F1" ) )
					wait .05;
		}
		devhelp_hudElements( hudarray, 0 );
		wait .05;
...
flag_set_trigger_specialops( trigger )
{
	...
	trigger.player_touched_arr = level._players;
...
flag_set_trigger_specialops_clear( flag )
{
	...
	while ( true )
	{
		...
		{
			self.player_touched_arr = level._players;
		}
	}
}
...
trigger_damage_flag_set( trigger )
{
	flag = trigger get_trigger_flag();
...
flag_set_coop_trigger( trigger )
{
	...
	for ( ;; )
	{
		...
		if ( agents.size == level._players.size )
			break;
	}
...
eq_trigger( trigger )
{
	level._set_eq_func[ true ] = ::set_eq_on;
	level._set_eq_func[ false ] = ::set_eq_off;
	...
	for ( ;; )
	{
		...
		for ( i = 0; i < ai.size; i++ )
		{
			ai[ i ] [[ level._set_eq_func[ ai[ i ] IsTouching( targ ) ] ]]();
		}
		while ( level._player IsTouching( trigger ) )
			wait( 0.05 );
...
get_trigger_eq_nums( num )
{
	...
	nums = [];
	nums[ 0 ] = num;
	for ( i = 0; i < level._eq_trigger_table[ num ].size; i++ )
	{
		nums[ nums.size ] = level._eq_trigger_table[ num ][ i ];
	}
	return nums;
}
...
ai_touched_eq_trigger( num, trigger )
{
	...
	for ( r = 0; r < nums.size; r++ )
	{
		if ( level._player.eq_table[ nums[ r ] ] )
		{
			self EQOff();
			break;
		}
	...
	for ( i = 0; i < touching.size; i++ )
	{
		...
		if ( level._player.eq_table[ touching[ i ] ] )
		{
...
ai_eq()
{
	level._set_eq_func[ false ] = ::set_eq_on;
	level._set_eq_func[ true ] = ::set_eq_off;
	index = 0;
	for ( ;; )
	{
		while ( !level._ai_array.size )
		{
			wait( 0.05 );
		}
		waittillframeend;
		waittillframeend;
		keys = GetArrayKeys( level._ai_array );
		index++;
		if ( index >= keys.size )
			index = 0;
		guy = level._ai_array[ keys[ index ] ];
		guy [[ level._set_eq_func[ SightTracePassed( level._player GetEye(), guy GetEye(), false, undefined ) ] ]]();
		wait( 0.05 );
	}
}
...
add_tokens_to_trigger_flags( tokens )
{
	...
	{
		flag = tokens[ i ];
		if ( !isdefined( level._trigger_flags[ flag ] ) )
		{
			level._trigger_flags[ flag ] = [];
		}

		level._trigger_flags[ flag ][ level._trigger_flags[ flag ].size ] = self;
	}
}
...
trigger_multiple_friendly_respawn( trigger )
{
	...
	{
		trigger waittill( "trigger" );
		level._respawn_spawner_org = origin;
...
{
	Assert( !isdefined( level._player_viewhand_model ) );	// only set this once per level
	AssertEx( IsSubStr( viewhandModel, "player" ), "Must set with viewhands_player_*" );
	level._player_viewhand_model = viewhandModel;
	PreCacheModel( level._player_viewhand_model );
}
...
trigger_hint( trigger )
{
	...
	if ( !isdefined( level._displayed_hints ) )
	{
		level._displayed_hints = [];
	}
	// give level script a chance to set the hint string and optional boolean functions on this hint
	waittillframeend;

	hint = trigger.script_hint;
	AssertEx( IsDefined( level._trigger_hint_string[ hint ] ), "Trigger_hint with hint " + hint + " had no hint string assigned to it. Define hint strings with add_hint_string()" );
	trigger waittill( "trigger", other );

	AssertEx( IsPlayer( other ), "Tried to do a trigger_hint on a non player entity" );

	if ( IsDefined( level._displayed_hints[ hint ] ) )
		return;
	level._displayed_hints[ hint ] = true;
...
stun_player( ADS_fraction )
{
	...
	if ( ADS_fraction > .3 )
	{
		if ( level._player.AllowAds == true )
			level._player PlaySound( "player_hit_while_ads" );

		level._player.AllowAds = false;
		level._player AllowAds( false );
	}
	level._player SetSpreadOverride( 20 );

	wait( GetDvarInt( "stuntime" ) );

	level._player AllowAds( true );
	level._player.AllowAds = true;
	level._player ResetSpreadOverride();
}
...
ammo_pickup( sWeaponType )
{
	...
	for ( ;; )
	{
		...
		weapons = level._player GetWeaponsListAll();
		...
		if ( level._player GetFractionMaxAmmo( weaponToGetAmmo ) >= 1 )
			continue;
	...
	if ( IsDefined( emptyActionSlotAmmo ) )
		level._player GiveWeapon( weaponToGetAmmo );	// this will only be for C4 and claymores if the player is totally out of them
	else
	{
		rounds = 1;
		...
		level._player SetWeaponAmmoStock( weaponToGetAmmo, level._player GetWeaponAmmoStock( weaponToGetAmmo ) + rounds );
	}

	level._player PlayLocalSound( "grenade_pickup" );
	trig Delete();
...
{
	wait( 0.05 );
	level._first_frame = -1;
}

no_crouch_or_prone_think( trigger )
{
	array_thread( level._players, ::no_crouch_or_prone_think_for_player, trigger );
}
...
{
	array_thread( level._players, ::no_prone_think_for_player, trigger );
}
...
check_flag_for_stat_tracking( msg )
{
	...
	[[ level._sp_stat_tracking_func ]]( msg );
}
...
{
	waittillframeend;
	if ( !isdefined( level._scr_model ) )
		return;
	models = GetArrayKeys( level._scr_model );
	for ( i = 0; i < models.size; i++ )
	{
		if ( IsArray( level._scr_model[ models[ i ] ] ) )
		{
			for ( modelIndex = 0; modelIndex < level._scr_model[ models[ i ] ].size; modelIndex++ )
				PreCacheModel( level._scr_model[ models[ i ] ][ modelIndex ] );
		}
		else
			PreCacheModel( level._scr_model[ models[ i ] ] );
	}
}
...
filmy()
{
	...
	looper = Spawn( "script_model", level._player GetEye() );
	...
	while ( 1 )
	{
		...
		looper.origin = level._player GetEye() + ( vector_multiply( AnglesToForward( level._player GetPlayerAngles() ), 50 ) );
	}
}
...
player_death_detection()
{
	...
	level._player add_wait( ::waittill_msg, "death" );
	do_wait_any();
	...
	SetDvar( "player_died_recently", recently_skill[ level._gameskill ] );
}
...
trigger_spawngroup( trigger )
{
	...
	if ( !isdefined( level._spawn_groups[ spawngroup ] ) )
		return;

	trigger waittill( "trigger" );

	spawners = random( level._spawn_groups[ spawngroup ] );
...
init_level_players()
{
	level._players = GetEntArray( "player", "classname" );
	for ( i = 0; i < level._players.size; i++ )
	{
		level._players[ i ].unique_id = "player" + i;
	}

	level._player = level._players[ 0 ];
	if ( level._players.size > 1 )
		level._player2 = level._players[ 1 ];
...
watchWeaponChange()
{
	if ( !isdefined( level._friendly_thermal_Reflector_Effect ) )
		level._friendly_thermal_Reflector_Effect = LoadFX( "misc/thermal_tapereflect_inverted" );
	self endon( "death" );
...
loop_friendly_thermal_Reflector_Effect( player_id, onlyForThisPlayer )
{
	...
	for ( ;; )
	{
		if ( IsDefined( onlyForThisPlayer ) )
			PlayFXOnTagForClients( level._friendly_thermal_Reflector_Effect, self, "J_Spine4", onlyForThisPlayer );
		else
			PlayFXOnTag( level._friendly_thermal_Reflector_Effect, self, "J_Spine4" );
...
handle_getviewpos()
{
	...
	while ( 1 )
	{
		...
		playerviewpos = level._player.origin + ( 0, 0, level._player GetPlayerViewHeight() ) ;
		playerviewangle = level._player GetPlayerAngles();
...
trigger_multiple_visionset( trigger )
{
	...
	for ( ;; )
	{
		...
		if ( IsDefined( trigger.script_sun_visionset ) )
		{
			if ( IsDefined( trigger.script_sun_direction ) )
			{
				set_sun_vision_set( trigger.script_sun_visionset, trigger.script_sun_delay, trigger.script_sun_intensity, trigger.script_sun_inner_fov, trigger.script_sun_outer_fov, trigger.script_sun_direction );
			}
			else if ( IsDefined( trigger.script_sun_outer_fov ) )
			{
				set_sun_vision_set( trigger.script_sun_visionset, trigger.script_sun_delay, trigger.script_sun_intensity, trigger.script_sun_inner_fov, trigger.script_sun_outer_fov );
			}
			else if ( IsDefined( trigger.script_sun_inner_fov ) )
			{
				set_sun_vision_set( trigger.script_sun_visionset, trigger.script_sun_delay, trigger.script_sun_intensity, trigger.script_sun_inner_fov );
			}
			else if ( IsDefined( trigger.script_sun_intensity  ) )
			{
				set_sun_vision_set( trigger.script_sun_visionset, trigger.script_sun_delay, trigger.script_sun_intensity );
			}
			else if ( IsDefined( trigger.script_sun_delay ) )
			{
				set_sun_vision_set( trigger.script_sun_visionset, trigger.script_sun_delay );
			}
			else
			{
				set_sun_vision_set( trigger.script_sun_visionset );
			}
		}
	}
}
...
trigger_multiple_compass( trigger )
{
	...
	if ( !isdefined( level._minimap_image ) )
		level._minimap_image = "";
	for ( ;; )
	{
		trigger waittill( "trigger" );
		if ( level._minimap_image != minimap_image )
		{
...
slowmo_system_defaults()
{
	level._slowmo.lerp_time_in = 0.0;
	level._slowmo.lerp_time_out = .25;
	level._slowmo.speed_slow = 0.4;
	level._slowmo.speed_norm = 1.0;
}
