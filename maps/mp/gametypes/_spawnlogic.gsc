...
addSpawnPoints( team, spawnPointName, isSetOptional )
{
	...
	oldSpawnPoints = [];
	if ( level._teamSpawnPoints[team].size )
		oldSpawnPoints = level._teamSpawnPoints[team];
	
	level._teamSpawnPoints[team] = getSpawnpointArray( spawnPointName );
	
	if ( !level._teamSpawnPoints[team].size && !isSetOptional )
	{
	...
	for ( index = 0; index < level._teamSpawnPoints[team].size; index++ )
	{
		spawnpoint = level._teamSpawnPoints[team][index];
		
		if ( !isdefined( spawnpoint.inited ) )
		{
			spawnpoint spawnPointInit();
			level._spawnpoints[ level._spawnpoints.size ] = spawnpoint;
		}
	}
	...
	for ( index = 0; index < oldSpawnPoints.size; index++ )
	{
		...
		level._spawnMins = expandMins( level._spawnMins, origin );
		level._spawnMaxs = expandMaxs( level._spawnMaxs, origin );
		
		level._teamSpawnPoints[team][ level._teamSpawnPoints[team].size ] = oldSpawnPoints[index];
	}
}
...
placeSpawnPoints( spawnPointName )
{
	...
	for( index = 0; index < spawnPoints.size; index++ )
	{
		...
		level._extraspawnpointsused[ level._extraspawnpointsused.size ] = spawnPoints[index];
		#/
	}
}
...
getSpawnpointArray( classname )
{
	...
	if ( !isdefined( level._extraspawnpoints ) || !isdefined( level._extraspawnpoints[classname] ) )
		return spawnPoints;
	
	for ( i = 0; i < level._extraspawnpoints[classname].size; i++ )
	{
		spawnPoints[ spawnPoints.size ] = level._extraspawnpoints[classname][i];
	}
...
expandSpawnpointBounds( classname )
{
	...
	for( index = 0; index < spawnPoints.size; index++ )
	{
		level._spawnMins = expandMins( level._spawnMins, spawnPoints[index].origin );
		level._spawnMaxs = expandMaxs( level._spawnMaxs, spawnPoints[index].origin );
	}
}
...
{
	return level._teamSpawnPoints[team];
}
...
{
	//TagZP<TODO> fix this, hacking at the moment just to get people spawing, will take a closer look soon for MTDM.
	otherTeam = "none";
		...
		if ( level._teambased )
		{
			if( level._multiteambased )
			{
				for( i = 0; i < level._teamNameList.size; i++ )
				{
					...
					{
						sights += bestspawnpoint.sights[level._teamNameList[i]];
					}
				}
			}
			else
...
//	level endon("game_ended");
		...
		if ( spawnpoint.numPlayersAtLastUpdate > 0 )
		{
			...
			if( level._multiteambased )
			{
				...
				foreach( teamname in level._teamNameList )
				{
					...
					{
						enemyDistSum += spawnpoint.distSum[ teamname ];
					}
				}
			}
			else
	...
	//Added to NX1, integrated from MW3
	if ( isDefined( self.lastDeathPos ) && level._gameType != "dom" )
...
getSpawnpoint_SafeSpawn( spawnpoints )
{
	...
	enemyTeam = "none";
	if ( !level._teambased || level._multiTeamBased )
	{
		enemyTeam = "all";
	}
	else
	...
	foreach ( spawnpoint in spawnpoints )
	{
		...
		foreach ( grenade in level._grenades )
		{
		...
		if ( isDefined( level._artilleryDangerCenters ) )
		{
...
Spawnlogic_Begin()
{
	...
	/#
	level._debugSpawning = (getdvarint("scr_spawnpointdebug") > 0);
	#/
}
...
init()
{
	...
	level._spawnlogic_deaths = [];
	// DEBUG
	level._spawnlogic_spawnkills = [];
	level._players = [];
	level._grenades = [];
	level._pipebombs = [];
	level._turrets = [];
	...
	level._teamSpawnPoints["axis"] = [];
	level._teamSpawnPoints["allies"] = [];
	
	if( level._multiTeamBased )
	{
		for( i = 0; i < level._teamNameList.size; i++ )
		{
			level._teamSpawnPoints[level._teamNameList[i]] = [];
		}
	}
	...
	level._spawnMins = (0,0,0);
	level._spawnMaxs = (0,0,0);
	if ( isdefined( level._safespawns ) )
	{
		for( i = 0; i < level._safespawns.size; i++ )
		{
			level._safespawns[i] spawnPointInit();
		}
	}
...
sightCheckCost()
{
	...
	for ( ;; )
	{
		...
		if ( traceType == "bullet" && isDefined( level._players[0] ) )
		{
			for ( i = 0; i < traceCount; i++ )
				bulletTracePassed( level._players[0].origin + (0,0,50), (0,0,0), false, undefined );		
			
		}
		else if ( traceType == "damagecone" && isDefined( level._players[0] ) )
		{
			for ( i = 0; i < traceCount; i++ )
				level._players[0] damageConeTrace( (0,0,0) );
		}
		else if ( traceType == "sightcone" && isDefined( level._players[0] ) )
		{
			for ( i = 0; i < traceCount; i++ )
				level._players[0] sightConeTrace( (0,0,0) );
		}
		else
		{
			wait ( 1.0 );
...
spawnProfile()
{
	...
	while(1)
	{
		...
		*/
		...
		spawnObj getSpawnpoint_NearTeam(level._spawnpoints);
		wait ( 0.05 );
	}
}
...
spawnGraph()
{
	...
	while(1)
	{
		...
		{
			if ( !level._players.size || !isdefined( level._players[0].team ) || level._players[0].team == "spectator" || !isdefined( level._players[0].class ) )
				break;
		...
		level._players[0] getSpawnpoint_NearTeam( fakespawnpoints );
		...
		level._players[0] drawSpawnGraph( fakespawnpoints, w, h, weightscale );
...
loopbotspawns()
{
	while(1)
	{
		...
		for (i = 0; i < level._players.size; i++)
		{
			...
			if ( level._players[i].sessionstate == "playing" && issubstr(level._players[i].name, "bot") )
			{
				bots[bots.size] = level._players[i];
			}
		}
		if ( bots.size > 0 )
		{
...
profileDebug()
{
	while(1)
	{
		...
		for (i = 0; i < level._spawnpoints.size; i++)
			level._spawnpoints[i].weight = randomint(10000);
		if (level._players.size > 0)
			level._players[randomint(level._players.size)] getSpawnpoint_NearTeam(level._spawnpoints);
...
trackGrenades()
{
	while ( 1 )
	{
		level._grenades = getentarray("grenade", "classname");
		wait .05;
	}
}
...
{
	pos = self.origin + level._claymoremodelcenteroffset;
	playerpos = playerorigin + (0,0,32);
	distsqrd = distancesquared(pos, playerpos);
	
	forward = anglestoforward(self.angles);
	
	if (distsqrd < level._claymoreDetectionRadius*level._claymoreDetectionRadius)
	{
		playerdir = vectornormalize(playerpos - pos);
		angle = acos(vectordot(playerdir, forward));
		if (angle < level._claymoreDetectionConeAngle) {
			return true;
		}
	}
	return false;
...
avoidWeaponDamage(spawnpoints)
{
	...
	for (i = 0; i < spawnpoints.size; i++)
	{
		for (j = 0; j < level._grenades.size; j++)
		{
			if ( !isdefined( level._grenades[j] ) )
				continue;

			// could also do a sight check to see if it's really dangerous.
			if (distancesquared(spawnpoints[i].origin, level._grenades[j].origin) < mingrenadedistsquared)
			{
		...
		cloudDanger = maps\mp\_gasgrenades::getCloudDanger( spawnpoints[i] ); // 0 = none, 1 = full, might be > 1 for more than 1 gascloudin proximity
		
		if ( cloudDanger > 0 )
		{
			worsen = cloudDanger * weaponDamagePenalty;
			spawnpoints[i].weight -= worsen;
			/#
			spawnpoints[i].spawnData[spawnpoints[i].spawnData.size] = "Was near gas (" + int(cloudDanger*100) + "% danger): -" + int(worsen);
			#/
		}
	}
...
spawnPerFrameUpdate()
{
	...
	while(1)
	{
		...
		spawnpointindex = (spawnpointindex + 1) % level._spawnPoints.size;
		
		if ( getdvar( "scr_spawnpoint_forceindex" ) != "" )
			spawnpointindex = getdvarint( "scr_spawnpoint_forceindex" );
		
		spawnpoint = level._spawnPoints[spawnpointindex];
...
spawnPointUpdate( spawnpoint )
{
	...
	if ( level._teambased )
	{
		...
		if( level._multiteambased )
		{
			for( i = 0; i < level._teamNameList.size; i++ )
			{
				spawnpoint.sights[level._teamNameList[i]] = 0;
			}
	...
	//adding values for multiteam
	if( level._multiTeamBased )
	{
		for( i = 0; i < level._teamNameList.size; i++ )
		{
			spawnpoint.distSum[level._teamNameList[i]] = 0;
			spawnpoint.weightedDistSum[level._teamNameList[i]] = 0;	
			spawnpoint.minDist[level._teamNameList[i]] = 9999999;
			totalPlayers[level._teamNameList[i]] = 0;
			weightSum[level._teamNameList[i]] = 0;
		}
	}
	...
	team = "all";
	teambased = level._teambased;
	multiTeamBased = level._multiTeamBased;
...
lastMinuteSightTraces( spawnpoint )
{
	...
	foreach ( player in level._players )
	{
		if ( player.team == self.team && level._teambased )
			continue;
		if ( player.sessionstate != "playing" )
			continue;
		if ( player == self )
	...
	if ( isdefined( closest ) )
	{
		...
		if ( sightValue > 0 )
		{
			...
			//TagZP<NOTE> moved this over from get best spawn point so that we can make sure that the sight value is going int the proper teams bucket
			sightValue = adjustSightValue( sightvalue );
			if ( level._teambased )
				spawnpoint.sights[closest.team] += sightValue;
			else
				spawnpoint.sights += sightValue;
	...
	if ( isdefined( secondClosest ) )
	{
		...
		if ( sightValue > 0 )
		{
			...
			sightValue = adjustSightValue( sightvalue );
			if ( level._teambased )
				spawnpoint.sights[secondClosest.team] += sightValue;
			else
				spawnpoint.sights += sightValue;
		...
		foreach ( spawnpoint in spawnpoints )
		{
			if( level._multiteambased )
			{
				...
				foreach( teamname in level._teamNameList )
				{
					if( teamname != self.team )
					{
						penalty += lospenalty * spawnpoint.sights[teamname];
					}
				}
			}
			else
	...
	foreach ( spawnpoint in spawnpoints )
	{
		...
		if( level._multiteambased )
		{
			foreach( teamname in level._teamNameList )
			{
				...
				{
					spawnpoint.weight -= spawnpoint.nearbyPenalty[teamname];
				}
			}
		}
		else
