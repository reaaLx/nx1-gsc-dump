...
isTeamSwitchBalanced()
{
	...
	{
		//TagZP<TODO> make multiteam check for team balance when players change teams.
		return true;
	}
...
handleTeamChangeDeath()
{
	...
	{
		self thread [[ level._onXPEvent ]]( "suicide" );
...
handleWorldDeath( attacker, lifeId, sMeansOfDeath, sHitLoc )
{
	...
	if ( level._teamBased && attacker.team != self.team )
	{
		if ( isDefined( level._onNormalDeath ) && isPlayer( attacker ) && attacker.team != "spectator" )
			[[ level._onNormalDeath ]]( self, attacker, lifeId );
	}
}
...
handleFriendlyFireDeath( attacker )
{
	...
	attacker thread [[ level._onXPEvent ]]( "teamkill" );
	...
	if ( maps\mp\gametypes\_tweakables::getTweakableValue( "team", "teamkillpointloss" ) )
	{
 	...
 	if ( level._maxAllowedTeamkills < 0 )
 		return;

	if ( level._inGracePeriod )
	{
		teamKillDelay = 1;
		attacker.pers["teamkills"] += level._maxAllowedTeamkills;
	}
	else if ( attacker.pers[ "teamkills" ] > 1 && getTimePassed() < ( (level._gracePeriod * 1000) + 8000 + ( attacker.pers[ "teamkills" ] * 1000 ) ) )
	{
		teamKillDelay = 1;
		attacker.pers["teamkills"] += level._maxAllowedTeamkills;
	}
...
handleNormalDeath( lifeId, attacker, eInflictor, sWeapon, sMeansOfDeath )
{
	...
	if ( maps\mp\killstreaks\_killstreaks::isEnemyAirKillstreakActive( attacker ))
		attacker incPlayerStat ( "killduringenemyairkillstreak", 1 );
	...
	if ( isAlive( attacker ) )
	{
		...
		if ( isDefined( level._killStreakSpecialCaseWeapons[sWeapon] ) ) // this is an optimization
		{
			if ( getdvarint( "bg_tallyKillstreakKills" ) == 1 )
			{
				switch ( sWeapon )
				{
					case "ac130_105mm_mp":
					...
					case "aresminigun_mp":
					case "manned_minigun_turret_mp":
					case "atbr_bullet_mp":
					case "nx_miniuav_rifle_player_mp":
	...
	if ( isDefined( level._ac130player ) && level._ac130player == attacker )
	...
	if ( isDefined( level._onNormalDeath ) && attacker.pers[ "team" ] != "spectator" )
		[[ level._onNormalDeath ]]( self, attacker, lifeId );
	...
	if ( isDefined( self.attackers ) )
	{
		foreach ( player in self.attackers )
		{
			...
			if( level._multiteambased )
			{
				if( attacker.pers["team"] == player.pers["team"] )
				{
...
QueueShieldForRemoval( shield )
{
	...
	if ( !isDefined( level._shieldTrashArray ) )
		level._shieldTrashArray = [];

	if ( level._shieldTrashArray.size >= MY_MAX_SHIELDS_AT_A_TIME )
	{
		idxMax = (level._shieldTrashArray.size - 1);
		level._shieldTrashArray[0] delete();
		for ( idx = 0; idx < idxMax; idx++ )
			level._shieldTrashArray[idx] = level._shieldTrashArray[idx + 1];
		level._shieldTrashArray[idxMax] = undefined;
	}

	level._shieldTrashArray[level._shieldTrashArray.size] = shield;
}


LaunchShield( damage, meansOfDeath )
{
	//printLn( "RIOT SHIELD TRACK LOCATION: LAUNCH SHIELD CALLED!!!" );
	if ( isDefined( self.hasRiotShield ) && self.hasRiotShield )
	{
		...
		// clear all the shield switching flags
		self maps\mp\gametypes\_class::initCheckHasRiotShield();
	}
}
...
PlayerKilled_internal( eInflictor, attacker, victim, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration, isFauxDeath )
{
	...
	if ( (!isDefined( attacker ) || attacker.classname == "trigger_hurt" || attacker.classname == "worldspawn" || attacker == victim ) && isDefined( self.attackers )  )
	{
		...
		foreach ( player in self.attackers )
		{
			...
			if ( player == victim || (level._teamBased && player.team == victim.team ) )
				continue;
	...
	victim thread [[ level._onPlayerKilled ]]( eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration, lifeId );
	...
	if ( isDefined( attacker.finalKill ) && doKillcam && !isDefined( level._nukeDetonated ) )
	{
	...
	if ( !isFauxDeath )
	{
		if ( !level._showingFinalKillcam && !level._killcam && doKillcam )
		{
	...
	if ( !(isDefined( victim.cancelKillcam) && victim.cancelKillcam) && doKillcam && level._killcam && game[ "state" ] == "playing" && !victim isUsingRemote() && !level._showingFinalKillcam )
	{
...
checkForceBleedout()
{
	if ( level._dieHardMode != 1 )
		return false;
	
	if ( !getGametypeNumLives() )
		return false;
	
	if ( level._livesCount[self.team] > 0 )
		return false;
...
{
	level._showingFinalKillcam = true;
...
HitlocDebug( attacker, victim, damage, hitloc, dflags )
{
	...
	if ( level._splitscreen || !isPLayer( attacker ) )
		return;
	...
	attacker.damageInfo[ 0 ].bp = ( dflags & level._iDFLAGS_PENETRATION );
...
{	
	...
	attackerIsHittingTeammate = level._teamBased && isDefined( eAttacker ) && ( victim != eAttacker ) && isDefined( eAttacker.team ) && ( victim.pers[ "team" ] == eAttacker.team );

	stunFraction = 0.0;

	if ( iDFlags & level._iDFLAGS_STUN )
	{
		stunFraction = 0.0;
	...
	{
		if ( attackerIsHittingTeammate && level._friendlyfire == 0 )
			return;
		...
		if ( sMeansOfDeath == "MOD_PISTOL_BULLET" || sMeansOfDeath == "MOD_RIFLE_BULLET" || sMeansOfDeath == "MOD_EXPLOSIVE_BULLET" && !attackerIsHittingTeammate )
		{
			...
			if ( victim.shieldBulletHits >= level._riotShieldXPBullets )
			{
		...
		if ( iDFlags & level._iDFLAGS_SHIELD_EXPLOSIVE_IMPACT )
		{
			...
			if ( !(iDFlags & level._iDFLAGS_SHIELD_EXPLOSIVE_IMPACT_HUGE) )
				iDamage *= 0.0;
		}
		else if ( iDFlags & level._iDFLAGS_SHIELD_EXPLOSIVE_SPLASH )
		{
	...
	else if ( (smeansofdeath == "MOD_MELEE") && ( victim _hasPerk( "specialty_armorvest" ) ) )   
	{
		if ( !(attackerIsHittingTeammate && (level._friendlyfire == 0)) )
		{
			victim shellShock( "melee_trophy_suit", 2.4 );
		}
	}

	if 	( !attackerIsHittingTeammate && (smeansofdeath == "MOD_MELEE") && ( eAttacker _hasPerk( "specialty_meleehack" ) ) )  
	{
		eAttacker thread maps\mp\killstreaks\_uav::MeleeUAV();
	}
	...
	if ( !isDefined( vDir ) )
		iDFlags |= level._iDFLAGS_NO_KNOCKBACK;
	...
	if ( !(iDFlags & level._iDFLAGS_NO_PROTECTION) )
	{
		// items you own don't damage you in FFA
		if ( !level._teamBased && attackerIsNPC && isDefined( eAttacker.owner ) && eAttacker.owner == victim )
		{
		...
		if ( attackerIsHittingTeammate )
		{
			...
			if ( level._friendlyfire == 0 || ( !isPlayer(eAttacker) && level._friendlyfire != 1 ) )// no one takes damage
			{
			...
			else if ( level._friendlyfire == 1 )// the friendly takes damage
			{
				if ( iDamage < 1 )
					iDamage = 1;
			...
			else if ( ( level._friendlyfire == 2 ) && isReallyAlive( eAttacker ) )// only the attacker takes damage
			{
			...
			else if ( level._friendlyfire == 3 && isReallyAlive( eAttacker ) )// both friendly and attacker take damage
			{
				iDamage = int( iDamage * .5 );
				if ( iDamage < 1 )
					iDamage = 1;
		...
		else// not hitting teammate
		{
			...
			if ( sMeansOfDeath == "MOD_EXPLOSIVE" || sMeansOfDeath == "MOD_GRENADE_SPLASH" )
			{
				if ( victim != eAttacker )
				{
					eAttacker notify( "explosive_hits" );
				}
			}
			...
			if ( isdefined( eAttacker ) )
				level._lastLegitimateAttacker = eAttacker;

			if ( isdefined( eAttacker ) && isPlayer( eAttacker ) && isDefined( sWeapon ) )
				eAttacker thread maps\mp\gametypes\_weapons::checkHit( sWeapon, victim, iDamage, sMeansOfDeath );
			...
			if ( isDefined( level._ac130player ) && isDefined( eAttacker ) && ( level._ac130player == eAttacker ) )
		...
		if ( isDefined( damager) && damager != victim && iDamage > 0 )
		{
			if ( iDFlags & level._iDFLAGS_STUN )
				typeHit = "stun";
			else if ( victim hasPerk("specialty_juggersuit", true) || victim _hasPerk( "specialty_netpainkiller" ) || (isExplosiveDamage( sMeansOfDeath ) && victim _hasPerk( "specialty_armorvest" )) )
...
addAttacker( victim, eAttacker, eInflictor, sWeapon, iDamage, vPoint, vDir, sHitLoc, psOffsetTime, sMeansOfDeath )
{
	...
	if ( maps\mp\gametypes\_weapons::isGrenadeWithLastingEffect( sWeapon ))
		victim.attackerData[ eAttacker.guid ].isGrenadeWithLastingEffect = true;
...
{
	if ( (self isUsingRemote() ) && (iDamage >= self.health) && !(iDFlags & level._iDFLAGS_STUN) )
	{
		if ( !isDefined( vDir ) )
			vDir = ( 0,0,0 );
...
{	
	...
	if ( isDefined( level._ac130player ) && isDefined( attacker ) && level._ac130player == attacker )
...
c4DeathDetonate()
{
	...
	self.c4DeathEffect = playFX( level._c4Death, self.origin );
...
lastStandTimer( delay, isFinalStand )
{
	...
	if ( !isFinalStand && !level._dieHardMode && ( !isDefined( self.inC4Death ) || !self.inC4Death ) )
	{
	...
	if ( level._dieHardMode == 1 && level._dieHardMode != 2 )
	{
...
personalUseBar( object )
{
	...
	lastRate = -1;
	while ( isReallyAlive( self ) && isDefined( object ) && object.inUse && !level._gameEnded && isDefined( self ) )
	{
