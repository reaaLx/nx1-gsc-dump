...
init()
{
	...
	precacheHelicopter( "nx_vehicle_attack_drone_mp", "cobra" );
	precacheHelicopter( "nx_vehicle_attack_drone_mp", "hind" );
	...
	level._chopper = undefined;

	// array of paths, each element is an array of start nodes that all leads to a single destination node
	level._heli_start_nodes = getEntArray( "heli_start", "targetname" );
	assertEx( level._heli_start_nodes.size, "No \"heli_start\" nodes found in map!" );

	level._heli_loop_nodes = getEntArray( "heli_loop_start", "targetname" );
	assertEx( level._heli_loop_nodes.size, "No \"heli_loop_start\" nodes found in map!" );

	level._heli_leave_nodes = getEntArray( "heli_leave", "targetname" );
	assertEx( level._heli_leave_nodes.size, "No \"heli_leave\" nodes found in map!" );

	level._heli_crash_nodes = getEntArray( "heli_crash_start", "targetname" );
	assertEx( level._heli_crash_nodes.size, "No \"heli_crash_start\" nodes found in map!" );
	
	level._heli_missile_rof 	= 5;	// missile rate of fire, one every this many seconds per target, could fire two at the same time to different targets
	level._heli_maxhealth 	= 1500;	// max health of the helicopter
	level._heli_debug 		= 0;	// debug mode, draws debugging info on screen
	
	level._heli_targeting_delay 	= 0.5;	// targeting delay
	level._heli_turretReloadTime = 1.5;	// mini-gun reload time
	level._heli_turretClipSize 	= 40;	// mini-gun clip size, rounds before reload
	level._heli_visual_range 	= 3500;	// distance radius helicopter will acquire targets (see)
			
	level._heli_target_spawnprotection 	= 5;		// players are this many seconds safe from helicopter after spawn
	level._heli_target_recognition 		= 0.5;		// percentage of the player's body the helicopter sees before it labels him as a target
	level._heli_missile_friendlycare 	= 256;		// if friendly is within this distance of the target, do not shoot missile
	level._heli_missile_target_cone 		= 0.3;		// dot product of vector target to helicopter forward, 0.5 is in 90 range, bigger the number, smaller the cone
	level._heli_armor_bulletdamage 		= 0.3;		// damage multiplier to bullets onto helicopter's armor
	
	level._heli_attract_strength 		= 1000;
	level._heli_attract_range 			= 4096;	
	
	level._heli_angle_offset 			= 90;
	level._heli_forced_wait 				= 0;

	// helicopter fx
	level._chopper_fx["explode"]["death"] = [];
	level._chopper_fx["explode"]["large"] = loadfx ("explosions/helicopter_explosion_secondary_small");
	level._chopper_fx["explode"]["medium"] = loadfx ("explosions/aerial_explosion");
	level._chopper_fx["smoke"]["trail"] = loadfx ("smoke/smoke_trail_white_heli");
	level._chopper_fx["fire"]["trail"]["medium"] = loadfx ("fire/fire_smoke_trail_L_emitter");
	level._chopper_fx["fire"]["trail"]["large"] = loadfx ("fire/fire_smoke_trail_L");

	level._chopper_fx["damage"]["light_smoke"] = loadfx ("smoke/smoke_trail_white_heli_emitter");
	level._chopper_fx["damage"]["heavy_smoke"] = loadfx ("smoke/smoke_trail_black_heli_emitter");
	level._chopper_fx["damage"]["on_fire"] = loadfx ("fire/fire_smoke_trail_L_emitter");

	level._chopper_fx["light"]["left"] = loadfx( "misc/aircraft_light_wingtip_green" );
	level._chopper_fx["light"]["right"] = loadfx( "misc/aircraft_light_wingtip_red" );
	level._chopper_fx["light"]["belly"] = loadfx( "misc/aircraft_light_red_blink" );
	level._chopper_fx["light"]["tail"] = loadfx( "misc/aircraft_light_white_blink" );

	level._fx_heli_dust = loadfx ("treadfx/heli_dust_default");
	level._fx_heli_water = loadfx ("treadfx/heli_water");
	...
	level._killstreakFuncs["helicopter"] = ::useHelicopter;
	level._killstreakFuncs["helicopter_blackbox"] = ::useHelicopterBlackbox;
	level._killstreakFuncs["helicopter_flares"] = ::useHelicopterFlares;
	level._killstreakFuncs["helicopter_minigun"] = ::useHelicopterMinigun;
	level._killstreakFuncs["helicopter_mk19"] = ::useHelicopterMK19;
	
	level._heliDialog["tracking"][0] = "ac130_fco_moreenemy";
	level._heliDialog["tracking"][1] = "ac130_fco_getthatguy";
	level._heliDialog["tracking"][2] = "ac130_fco_guyrunnin";
	level._heliDialog["tracking"][3] = "ac130_fco_gotarunner";
	level._heliDialog["tracking"][4] = "ac130_fco_personnelthere";
	level._heliDialog["tracking"][5] = "ac130_fco_rightthere";
	level._heliDialog["tracking"][6] = "ac130_fco_tracking";

	level._heliDialog["locked"][0] = "ac130_fco_lightemup";
	level._heliDialog["locked"][1] = "ac130_fco_takehimout";
	level._heliDialog["locked"][2] = "ac130_fco_nailthoseguys";

	level._lastHeliDialogTime = 0;	
...
{
	level._chopper_fx["explode"]["death"][ heliType ] = loadFx( deathFX );
	level._lightFxFunc[ heliType ] = lightFXFunc;
}

addAirExplosion( heliType, explodeFx )
{
	level._chopper_fx["explode"]["air_death"][ heliType ] = loadFx( explodeFx );
}


pavelowLightFX()
{
	playFXOnTag( level._chopper_fx["light"]["left"], self, "tag_light_L_wing1" );
	wait ( 0.05 );
	playFXOnTag( level._chopper_fx["light"]["right"], self, "tag_light_R_wing1" );
	wait ( 0.05 );
	playFXOnTag( level._chopper_fx["light"]["belly"], self, "tag_light_belly" );
	wait ( 0.05 );
	playFXOnTag( level._chopper_fx["light"]["tail"], self, "tag_light_tail" );
	wait ( 0.05 );
	playFXOnTag( level._chopper_fx["light"]["tail"], self, "tag_light_tail2" );
	wait ( 0.05 );
	playFXOnTag( level._chopper_fx["light"]["belly"], self, "tag_light_cockpit01" );
}


defaultLightFX()
{
	playFXOnTag( level._chopper_fx["light"]["left"], self, "tag_light_L_wing" );
	wait ( 0.05 );
	playFXOnTag( level._chopper_fx["light"]["right"], self, "tag_light_R_wing" );
	wait ( 0.05 );
	playFXOnTag( level._chopper_fx["light"]["belly"], self, "tag_light_belly" );
	wait ( 0.05 );
	playFXOnTag( level._chopper_fx["light"]["tail"], self, "tag_light_tail" );
...
tryUseHelicopter( lifeId, heliType )
{
	...
	if ( (!isDefined( heliType ) || heliType == "flares") && isDefined( level._chopper ) )
	{
	...
	else if ( isDefined( level._chopper ) )
	{
...
startHelicopter( lifeId, heliType )
{
	...
	startNode = level._heli_start_nodes[ randomInt( level._heli_start_nodes.size ) ];
...
precacheHelicopter( model, heliType )
{
	...
	level._heli_types[model] = heliType;
	...
	// helicopter sounds:
	level._heli_sound["allies"]["hit"] = "cobra_helicopter_hit";
	level._heli_sound["allies"]["hitsecondary"] = "cobra_helicopter_secondary_exp";
	level._heli_sound["allies"]["damaged"] = "cobra_helicopter_damaged";
	level._heli_sound["allies"]["spinloop"] = "cobra_helicopter_dying_loop";
	level._heli_sound["allies"]["spinstart"] = "cobra_helicopter_dying_layer";
	level._heli_sound["allies"]["crash"] = "cobra_helicopter_crash";
	level._heli_sound["allies"]["missilefire"] = "weap_cobra_missile_fire";
	level._heli_sound["axis"]["hit"] = "cobra_helicopter_hit";
	level._heli_sound["axis"]["hitsecondary"] = "cobra_helicopter_secondary_exp";
	level._heli_sound["axis"]["damaged"] = "cobra_helicopter_damaged";
	level._heli_sound["axis"]["spinloop"] = "cobra_helicopter_dying_loop";
	level._heli_sound["axis"]["spinstart"] = "cobra_helicopter_dying_layer";
	level._heli_sound["axis"]["crash"] = "cobra_helicopter_crash";
	level._heli_sound["axis"]["missilefire"] = "weap_cobra_missile_fire";
	
	
	/* 
	// TagZP<NOTE> this seems like a waste of memory, if we really do want unique versions of these sounds in MTDM 
	// we can have them but for now, im not going to create this large 2 dimensional array of strings.
	if( level._multiTeamBased )
	{
		//stubbing in allies sounds for now
		foreach( teamname in level._teamNameList )
		{
			level._heli_sound[teamname]["hit"] = "cobra_helicopter_hit";
			level._heli_sound[teamname]["hitsecondary"] = "cobra_helicopter_secondary_exp";
			level._heli_sound[teamname]["damaged"] = "cobra_helicopter_damaged";
			level._heli_sound[teamname]["spinloop"] = "cobra_helicopter_dying_loop";
			level._heli_sound[teamname]["spinstart"] = "cobra_helicopter_dying_layer";
			level._heli_sound[teamname]["crash"] = "cobra_helicopter_crash";
			level._heli_sound[teamname]["missilefire"] = "weap_cobra_missile_fire";
		}
	}
	*/
}
...
spawn_helicopter( owner, origin, angles, vehicleType, modelName )
{
	...
	chopper.heli_type = level._heli_types[ modelName ];
	
	chopper thread [[ level._lightFxFunc[ chopper.heli_type ] ]]();
	
	chopper addToHeliList();
		
	chopper.zOffset = (0,0,chopper getTagOrigin( "tag_origin" )[2] - chopper getTagOrigin( "tag_ground" )[2]);
	chopper.attractor = Missile_CreateAttractorEnt( chopper, level._heli_attract_strength, level._heli_attract_range );
...
weaponLockThink( chopper )
{
	...
	if ( !isDefined( level._heliTargetOrigin ) )
	{
		level._heliTargetOrigin = spawn( "script_origin", (0,0,0) );
		level._heliTargetOrigin hide();
	}

	for ( ;; )
	{
		trace = bulletTrace( self getEye(), self getEye() + (anglesToForward( self getPlayerAngles() ) * 100000 ), 1, self );
		level._heliTargetOrigin.origin = trace["position"];
...
{
	if ( getTime() - level._lastHeliDialogTime < 6000 )
		return;
	
	level._lastHeliDialogTime = getTime();
	
	randomIndex = randomInt( level._heliDialog[ dialogGroup ].size );
	soundAlias = level._heliDialog[ dialogGroup ][ randomIndex ];
...
heli_think( lifeId, owner, startnode, heli_team, heliType )
{
	...
	switch( heliType )
	{
		...
		default:
			...
			if ( owner.team == "allies" )
				vehicleModel = "nx_vehicle_attack_drone_mp";
			else
				vehicleModel = "nx_vehicle_attack_drone_mp";
			break;		
	}
	...
	if ( heliType == "flares" )
		chopper.maxhealth = level._heli_maxhealth*2;			// max health
	else
		chopper.maxhealth = level._heli_maxhealth;			// max health

	chopper.targeting_delay = level._heli_targeting_delay;		// delay between per targeting scan - in seconds
	...
	chopper.hasBeenDestroyed = false;                   // new member data to prevent multiple players getting killstreak destroyed by shooting at destroyed helicopter
	...
	//attackAreas = [];
	loopNode = level._heli_loop_nodes[ randomInt( level._heli_loop_nodes.size ) ];	
	...
	{		
		...
		if ( isDefined( level._harriers ) )
		{
			foreach( harrier in level._harriers )
			{
				...
				if ( (level._teamBased && harrier.team != self.team) || (!level._teamBased && harrier.owner != self.owner) )
				{
...
canTarget_turret( player )
{
	...
	if ( distance( player.origin, self.origin ) > level._heli_visual_range )
		return false;
	
	if ( level._teamBased && player.pers["team"] == self.team )
		return false;
	...
	if ( player sightConeTrace( heli_turret_point, self) < level._heli_target_recognition )
		return false;	
...
update_player_threat( player )
{
	...
	player.threatlevel += ( (level._heli_visual_range - dist)/level._heli_visual_range )*100; // inverse distance % with respect to helicopter targeting range
...
heli_damage_monitor()
{
	...
	for( ;; )
	{
		...
		if ( isPlayer( attacker ) )
		{
			...
			{
				damage *= level._heli_armor_bulletdamage;
...
heli_health()
{
	...
	for ( ;; )
	{
		if ( self.damageTaken >= (self.maxhealth * 0.33) && damageState == 3 )
		{
			...
			playFxOnTag( level._chopper_fx["damage"]["light_smoke"], self, "tag_engine_left" );
		}
		else if ( self.damageTaken >= (self.maxhealth * 0.66) && damageState == 2 )
		{
			...
			stopFxOnTag( level._chopper_fx["damage"]["light_smoke"], self, "tag_engine_left" );
			playFxOnTag( level._chopper_fx["damage"]["heavy_smoke"], self, "tag_engine_left" );
		}
		else if( self.damageTaken > self.maxhealth )
		{
			...
			stopFxOnTag( level._chopper_fx["damage"]["heavy_smoke"], self, "tag_engine_left" );
			...
			{
				playFxOnTag( level._chopper_fx["damage"]["on_fire"], self, "tag_engine_left" );
...
heli_crash()
{
	...
	crashNode = level._heli_crash_nodes[ randomInt( level._heli_crash_nodes.size ) ];	
...
heli_secondary_explosions()
{
	...
	playFxOnTag( level._chopper_fx["explode"]["large"], self, "tag_engine_left" );
	self playSound ( level._heli_sound[teamname]["hitsecondary"] );
...
heli_spin( speed )
{
	...
	self playSound ( level._heli_sound[teamname]["hit"] );
...
spinSoundShortly()
{
	...
	self stopLoopSound();
	wait .05;
	self playLoopSound( level._heli_sound[teamname]["spinloop"] );
	wait .05;
	self playLoopSound( level._heli_sound[teamname]["spinstart"] );
}
...
heli_explode( altStyle )
{
	...
	if ( isDefined( altStyle ) && isDefined( level._chopper_fx["explode"]["air_death"][self.heli_type] ) )
	{
		deathAngles = self getTagAngles( "tag_deathfx" );
		
		playFx( level._chopper_fx["explode"]["air_death"][self.heli_type], self getTagOrigin( "tag_deathfx" ), anglesToForward( deathAngles ), anglesToUp( deathAngles ) );
	...
	else
	{
		...
		playFx( level._chopper_fx["explode"]["death"][self.heli_type], org, forward );
	}
	
	// play heli explosion sound
	teamname = self heli_getTeamForSoundClip();
	self playSound( level._heli_sound[teamname]["crash"] );
		...
		if ( isdefined( self.secondaryTarget ) )
		{
			...
			while( isdefined( self.missileTarget ) && isalive( self.missileTarget ) )
			{
				...
				if( self missile_target_sight_check( self.missileTarget ) )
					self thread missile_support( self.missileTarget, level._heli_missile_rof);
				else
					break;
...
missile_target_sight_check( missiletarget )
{
	...
	if ( heli_dot_target >= level._heli_missile_target_cone )
	{
...
attack_primary()
{
	...
	while ( 1 )
	{
		...
		for ( i = 0; i < level._heli_turretClipSize; i++ )
		{
			self setVehWeapon( "cobra_20mm_mp" );
			self fireWeapon( "tag_flash" );

			if ( i < level._heli_turretClipSize - 1 )
				wait weaponShootTime;
...
travelToNode( goalNode )
{
	...
	if ( originOffets["start"] != self.origin )
	{
		...
		// calculate ideal yaw
		self setgoalyaw( goalNode.angles[ 1 ] + level._heli_angle_offset );
...
heli_fly_well( destNodes )
{
	...
	for ( ;; )	
	{
		...
		self setgoalyaw( currentNode.angles[ 1 ] + level._heli_angle_offset );	

		if ( level._heli_forced_wait != 0 )
		{
			self waittill( "near_goal" ); //self waittillmatch( "goal" );
			wait ( level._heli_forced_wait );			
		}
...
heli_leave()
{
	...
	leaveNode = level._heli_leave_nodes[ randomInt( level._heli_leave_nodes.size ) ];
...
{
	if ( isdefined( level._heli_debug ) && level._heli_debug == 1.0 )
...
{
	if ( isdefined( level._heli_debug ) && level._heli_debug == 1.0 && !isdefined( frames ) )
	{
		thread draw_line( from, to, color );
	}
	else if ( isdefined( level._heli_debug ) && level._heli_debug == 1.0 )
...
addToHeliList()
{
	level._helis[self getEntityNumber()] = self;	
}

removeFromHeliList( entityNumber )
{
	level._helis[entityNumber] = undefined;
}	
