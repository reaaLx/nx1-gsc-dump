...
beginHarrier( lifeId, startPoint, pos )
{
	...
	else if( isDefined( level._airstrikeHeightScale ) )
		trueHeight = 850 * level._airstrikeHeightScale;
	else
		trueHeight = 850;
...
getCorrectHeight( x, y, rand )
{
	...
	if( isDefined( level._airstrikeHeightScale ) && trueHeight < ( 850 * level._airstrikeHeightScale ) )
		trueHeight = ( 950 * level._airstrikeHeightScale );
...
spawnDefensiveHarrier( lifeId, owner, pathStart, pathGoal )
{
	...
	level._harriers = remove_undefined_from_array( level._harriers );
	
	level._harriers[level._harriers.size] = harrier;
...
harrierLeave()
{
	...
	level._airPlane[level._airPlane.size - 1] = undefined; 
...
getNewPoint( pos, targ )
{
	...
	if ( !isDefined( targ ) )
	{
		...
		{
			center = level._mapCenter;
			movementDist = ( level._mapSize / 6 ) - 200; 
...
playHarrierFx()
{
	...
	wait( 0.2 );
	playfxontag( level._harrier_afterburnerfx, self, "tag_engine_right" );
	playfxontag( level._harrier_afterburnerfx, self, "tag_engine_left" );
	wait( 0.2 );
	playfxontag( level._harrier_afterburnerfx, self, "tag_engine_right2" );
	playfxontag( level._harrier_afterburnerfx, self, "tag_engine_left2" );
	wait( 0.2 );
...
stopHarrierWingFx()
{
	stopfxontag( level._fx_airstrike_contrail, self, "tag_right_wingtip" );
	stopfxontag( level._fx_airstrike_contrail, self, "tag_left_wingtip" );
}
...
harrierGetTargets()
{
	...
	for ( ;; )
	{
		...
		{	
			if ( !isDefined( level._chopper.nonTarget ) || ( isDefined( level._chopper.nonTarget ) && !level._chopper.nonTarget )  )
			{
					self thread engageVehicle( level._chopper );
					return;
			}
			else
			{
...
{
	if ( ( attacker == self || ( isDefined( attacker.pers ) && attacker.pers["team"] == self.team ) && level._teamBased ) && ( attacker != self.owner ) )
		return;
	...
	{
		damageAdd = int( damage*level._armorPiercingMod );
		damage += damageAdd;
	}
...
playDamageEfx()
{
	...
	stopFxOnTag( level._harrier_afterburnerfx, self, "tag_engine_left" );
	playFxOnTag( level._harrier_smoke, self, "tag_engine_left" );
	
	stopFxOnTag( level._harrier_afterburnerfx, self, "tag_engine_right" );
	playFxOnTag( level._harrier_smoke, self, "tag_engine_right" );
	wait( 0.15 );
	
	stopFxOnTag( level._harrier_afterburnerfx, self, "tag_engine_left2" );
	playFxontag( level._harrier_smoke, self, "tag_engine_left2" );
		
	stopFxOnTag( level._harrier_afterburnerfx, self, "tag_engine_right2" );
	playFxontag( level._harrier_smoke, self, "tag_engine_right2" );
...
harrierExplode()
{
	...
	playFx( level._harrier_deathfx, self getTagOrigin( "tag_deathfx" ), anglesToForward( deathAngles ), anglesToUp( deathAngles ) );
...
harrierSpin( speed )
{
	...
	playfxontag( level._chopper_fx["explode"]["medium"], self, "tag_origin" );
