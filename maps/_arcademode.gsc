...
main()
{
	...
	level._color_cool_green = ( 0.8, 2.0, 0.8 );
	level._color_cool_green_glow = ( 0.3, 0.6, 0.3 );

	// the colors used for the kill streak guys
	arcadeMode_init_kill_streak_colors();

	level._arcadeMode_checkpoint_dvars = [];
	level._arcadeMode_checkpoint_max = 10;
	level._arcadeMode_kills_hud = [];
	level._arcademode_kill_streak_ends = 0;
	level._arcadeMode_last_streak_time = 0;
	level._arcadeMode_ramping_score = 0;
	level._arcadeMode_new_kill_streak_allowed = true;
	flag_init( "arcadeMode_multiplier_maxed" );

	setdvar( "arcademode_lives_changed", 0 );

	level._arcadeMode_kill_streak_current_multiplier = 1;
	level._arcadeMode_kill_streak_multiplier_count = 3;// gotta kill these number of guys to get the multiplier
	arcadeMode_reset_kill_streak();

	for ( i = 0; i < level._arcadeMode_checkpoint_max; i++ )
	{
		setdvar( "arcademode_checkpoint_" + i, "" );
	}

	level._arcadeMode_last_multi_kill_sound = 0;
	level._arcadeMode_success = false;
	...
	level._global_kill_func = ::player_kill;
	level._global_damage_func_ads = ::player_damage_ads;
	level._global_damage_func = ::player_damage;

	level._arcadeMode_hud_sort = 50;

	level._arcadeMode_maxlives = 10;
	level._arcademode_rewarded_lives = 0;
	if ( getdvar( "arcademode_lives" ) == "" || getdvar( "arcademode_full" ) != "1" || level._script == "cargoship" )
	{
		setdvar( "arcademode_lives", 2 );
		level._arcademode_rewarded_lives = 2;
	}

	if ( getdvar( "arcademode_full" ) == "1" && level._script == "cargoship" )
	{
		setdvar( "arcademode_lives", 5 );
		level._arcademode_rewarded_lives = 5;
	}
	...
	level._arcademode_playthrough = getdvarint( "arcademode_playthrough_count" );
	level._arcademode_playthrough++ ;
	setdvar( "arcademode_playthrough_count", level._arcademode_playthrough );
	...
	*/	
	...
	if ( getdvar( "arcademode_combined_score" ) == "" || ( getdvar( "arcademode_full" ) == "1" && level._script == "cargoship" ) )
	{
...
//	set_next_extra_life( 0 );
	...
	timer *= 60;
	level._arcdeMode_startTime = gettime();
	level._arcadeMode_time = timer;
	level._arcadeMode_killBase = 50;// base points for a kill
	level._arcadeMode_damageBase = 5;// base points for damage

	level._arcadeMode_multiKills = [];
	mods = getarraykeys( level._arcadeMode_weaponMultiplier );
	for ( i = 0; i < mods.size; i++ )
	{
		level._arcadeMode_multiKills[ mods[ i ] ] = [];
	}
	blank_array = level._arcadeMode_multiKills;
...
//	thread arcademode_update_streak_progress();
	...
	for ( ;; )
	{
		...
		mods = getarraykeys( level._arcadeMode_multiKills );
		array_levelthread( mods, ::arcadeMode_add_points_for_mod );
		level._arcadeMode_multiKills = blank_array;
	}
}
...
arcademode_get_level_time()
{
	...
	*/
	...
	if ( isdefined( timer[ level._script ] ) )
		time_remaining = timer[ level._script ];
	...
	*/
	...
	level._arcademode_difficultyTimerScale = difficultyTimerScale;
...
arcadeMode_death_detection()
{
	...
	level._arcadeMode_redraw_score = true;
	...
	if ( lives < 0 )
	{
		wait 1.5;
		// GAME OVER
		level._arcadeMode_failureString = &"SCRIPT_GAME_OVER";
...
arcademode_update_timer()
{
    ...
    hud_time.x = 0;
    hud_time.y = 60;
    hud_time.sort = level._arcadeMode_hud_sort;
...
//	hud_time SetPulseFX( 30, 100000, 700 );//something, decay start, decay duration
 	...
 	level._arcadeMode_hud_timer = hud_time;
 	level endon( "arcadeMode_remove_timer" );

	timer = level._arcadeMode_time;
	hud_time settimer( timer - .1 );
	wait( timer );
	// TIME UP
	level._arcadeMode_failureString = &"SCRIPT_TIME_UP";
	thread arcadeMode_ends();
}
...
arcademode_update_lives()
{
	...
	level._arcadeMode_lives_hud = [];
	for ( i = 0; i < level._arcadeMode_maxlives; i++ )
	{
		arcademode_add_life( i, 	16, 78, 	 - 18, 64, level._arcadeMode_hud_sort );
	}
	...
	for ( ;; )
	{
		...
		if ( change != 0 )
		{
			...
			if ( change == -1 )
			{
				...
				level._arcademode_kill_streak_ends = gettime();
				thread arcademode_add_kill_streak_time( 0 );
				level._arcadeMode_new_kill_streak_allowed = false;
...
arcadeMode_redraw_life( index, earned_lives )
{
	...
	self.alpha = 1;
	self.glowAlpha = 1;
	self.color = level._color_cool_green;
}
...
arcademode_redraw_lives( lives )
{
	...
	for ( i = 0; i < lives; i++ )
	{
		level._arcadeMode_lives_hud[ i ] arcadeMode_redraw_life( i, earned_lives );
	}

	for ( i = lives; i < level._arcadeMode_maxlives; i++ )
	{
		if ( i < 0 )
			continue;
		if ( i >= 10 )
		...
		level._arcadeMode_lives_hud[ i ] arcadeMode_remove_life( i );
	}
}
...
arcademode_redraw_streak_progress()
{
	for ( i = 0; i < level._arcadeMode_kill_streak_current_count; i++ )
	{
		if ( i >= level._arcadeMode_kills_hud.size )
			return;

		level._arcadeMode_kills_hud[ i ].color = level._arcadeMode_streak_color[ level._arcadeMode_kill_streak_current_multiplier - 1 ];
		level._arcadeMode_kills_hud[ i ].glowColor = level._arcadeMode_streak_glow[ level._arcadeMode_kill_streak_current_multiplier - 1 ];
	}

	count = 0;
	for ( ;; )
	{
		color = level._arcadeMode_kill_streak_current_multiplier + count;
		if ( color >= level._arcadeMode_streak_color.size )
		{
			// go crazy would start here instead of this
			color = level._arcadeMode_streak_color.size - 1;
		}

		// sets the color of all the kill progression icons
		for ( i = level._arcadeMode_kill_streak_current_count + count * level._arcadeMode_kill_streak_multiplier_count; i < level._arcadeMode_kill_streak_current_count + ( count + 1 ) * level._arcadeMode_kill_streak_multiplier_count; i++ )
		{
			if ( i >= level._arcadeMode_kills_hud.size )
				return;

			level._arcadeMode_kills_hud[ i ].color = level._arcadeMode_streak_color[ color ];
			level._arcadeMode_kills_hud[ i ].glowColor = level._arcadeMode_streak_glow[ color ];
		}
		count++ ;
	}
}
...
arcademode_add_kill( num, x, y, offset, scale, sort )
{
	...
	hud_kill.x = x + num * offset;
 	if ( level._arcadeMode_kills_hud.size == 0 )
 	{
 		// store the location of the rightermost kill hud elem for reference later
 		level._arcademode_kill_zero_x_location = hud_kill.x;
 	}
	...
	hud_kill.sort = sort;
	hud_kill.color = level._color_cool_green;
	hud_kill.glowColor = level._color_cool_green_glow;
	hud_kill.glowAlpha = 1;
 	hud_kill.hidewheninmenu = true;

 	leftermost = false;
 	level._arcadeMode_kills_hud[ level._arcadeMode_kills_hud.size ] = hud_kill;
 	if ( level._arcadeMode_kills_hud.size == 10 )
 	{
 		leftermost = true;
		hud_kill.alpha = 0;
 	}
 	...
 	for ( ;; )
 	{
	 	if ( hud_kill.x == level._arcademode_kill_zero_x_location )
	 	{
	 		// all the way on the right
	 		decay_time = 4;
	 		if ( level._arcadeMode_kills_hud.size == 1 )
	 		{
			...
			for ( i = 0; i < level._arcadeMode_kills_hud.size - 1; i++ )
			{
				level._arcadeMode_kills_hud[ i ] = level._arcadeMode_kills_hud[ i + 1 ];
			}
			level._arcadeMode_kills_hud[ level._arcadeMode_kills_hud.size - 1 ] = undefined;

			if ( !level._arcadeMode_kills_hud.size )
...
get_streak_hud( x, y, width, height )
{
	...
	streak_hud.color = level._color_cool_green;
	streak_hud.sort = level._arcadeMode_hud_sort - 1;
//	streak_hud.alpha = 0.8;
	streak_hud.alpha = 0;
	streak_hud.glowColor = level._color_cool_green_glow;
...
{
	if ( !level._arcadeMode_new_kill_streak_allowed )
		return;

	level notify( "arcademode_new_kill_streak_time" );
	level endon( "arcademode_new_kill_streak_time" );

	if ( level._arcademode_kill_streak_ends < gettime() )
	{
		level._arcademode_kill_streak_ends = gettime() + timer * 1000;
	}
	else
	{
		level._arcademode_kill_streak_ends += timer * 1000;
	}
	waittillframeend;

	if ( isdefined( level._arcademode_hud_streak ) )
	{
		level._arcademode_hud_streak fadeovertime( 0.05 );
		level._arcademode_hud_streak.alpha = 1;
	}

	height = 26;
	width_mult = 12;
	max_timer = 90;
	streak_hud = level._arcadeMode_streak_hud;
	streak_hud_shadow = level._arcadeMode_streak_hud_shadow;
	width = level._arcademode_kill_streak_ends - gettime();
	width *= 0.001;
	if ( width > max_timer )
		width = max_timer;
	width *= width_mult;
...
//		streak_hud_shadow.alpha = 0.3;
	...
	level._arcadeMode_streak_hud = streak_hud;
	level._arcadeMode_streak_hud_shadow = streak_hud_shadow;

	streak_hud endon( "death" );

	timer = level._arcademode_kill_streak_ends - gettime();
	timer *= 0.001;
	drawTimer = int( timer );
	...
	for ( ;; )
	{
		...
		if ( isdefined( level._arcademode_hud_streak ) )
		{
			...
			level._arcademode_hud_streak fadeovertime( 1 );
			level._arcademode_hud_streak.alpha = ( time_remaining - 1 ) / 5;
		}
...
arcademode_add_kill_streak()
{
	...
	level._arcadeMode_kill_streak_current_count -- ;
	streak_time = gettime();

	if ( level._arcadeMode_kill_streak_current_count <= 0 && streak_time > level._arcadeMode_last_streak_time )
	{
		level._arcadeMode_last_streak_time = streak_time;
		old_mult = level._arcadeMode_kill_streak_current_multiplier;
		level._arcadeMode_kill_streak_current_multiplier++ ;

		if ( level._arcadeMode_kill_streak_current_multiplier >= level._arcadeMode_streak_color.size )
		{
			// go crazy would start here instead of this
			level._arcadeMode_kill_streak_current_multiplier = level._arcadeMode_streak_color.size;
			thread arcadeMode_multiplier_maxed();
		}

		if ( old_mult != level._arcadeMode_kill_streak_current_multiplier )
		{
			...
			level._player playsound( "arcademode_" + level._arcadeMode_kill_streak_current_multiplier + "x" );
			thread arcademode_draw_multiplier();
		}

		// reset the # of kills to get the next streak bonus
		level._arcadeMode_kill_streak_current_count = level._arcadeMode_kill_streak_multiplier_count;
	}
...
streak_timer_color_pulse()
{
	...
	waittillframeend;
	level._arcadeMode_streak_hud endon( "death" );
	for ( ;; )
	{
		timer = randomfloatrange( 0.1, 1.0 );
		level._arcadeMode_streak_hud fadeovertime( timer );
		level._arcadeMode_streak_hud.color = ( randomfloat( 1 ), randomfloat( 1 ), randomfloat( 1 ) );
		wait( timer );
	}
}
...
arcadeMode_multiplier_maxed()
{
	...
	streak_time = 20;
	level._arcademode_kill_streak_ends = gettime() + streak_time * 1000;
	thread arcademode_add_kill_streak_time( 0 );
	...
	MusicStop();
	if ( isdefined( level._last_song ) )
	{
		wait( 0.05 );
		MusicPlay( level._last_song );
	}
}
...
//	hud_score setText( "16750" );
	...
	hud_score.sort = level._arcadeMode_hud_sort;
	hud_score.glowColor = level._color_cool_green_glow;
...
arcademode_update_score()
{
	...
	level._arcademode_hud_digits = 10;
	level._arcademode_hud_scores = [];

	for ( i = 0; i < level._arcademode_hud_digits; i++ )
	{
		level._arcademode_hud_scores[ level._arcademode_hud_scores.size ] = get_hud_score();
		level._arcademode_hud_scores[ level._arcademode_hud_scores.size - 1 ].x = i * - 30;
	}
...
//	level.arcadeMode_hud_score setValue( 0 );
	...
	*/
	...
	hud_draw_score( score );
 	...
 	level._arcadeMode_redraw_score = false;

	for ( ;; )
	{
		wait( 0.05 );
		UpdateScoreElemsOnce();
		if ( level._arcadeMode_redraw_score )
		{
			level._arcadeMode_redraw_score = false;
		}
	}
...
hud_update_score( score_dvar )
{
	...
	if ( level._arcadeMode_redraw_score )
	{
		level._arcadeMode_ramping_score = score;
		hud_draw_score( score );
		return;
	}

	if ( level._arcadeMode_ramping_score >= score )
		return;

	// score grew, so increase the displayed score
	difference = score - level._arcadeMode_ramping_score;
	boost = difference * 0.2 + 1;
	if ( difference <= 15 )
		boost = 1;
	level._arcadeMode_ramping_score += boost;

	if ( level._arcadeMode_ramping_score > score )
		level._arcadeMode_ramping_score = score;
	hud_draw_score( int( level._arcadeMode_ramping_score ) );
}
...
hud_draw_score( score )
{
	hud_draw_score_for_elements( score, level._arcademode_hud_scores );
}
...
arcademode_add_life( num, x, y, offset, scale, sort )
{
	...
	hud_score.vertAlign = "top";
	hud_score.sort = sort;
	hud_score.color = level._color_cool_green;
	hud_score.glowColor = level._color_cool_green_glow;
	hud_score.glowAlpha = 0;
	hud_score.alpha = 0;
 	hud_score.hidewheninmenu = true;
 	level._arcadeMode_lives_hud[ level._arcadeMode_lives_hud.size ] = hud_score;
}
...
arcadeMode_define_damage_multipliers()
{
	...
	*/
	...
	level._arcadeMode_weaponMultiplier = damage_multipliers;
	...
	level._arcadeMode_weaponBonus = damage_bonus;
	...
	level._arcadeMode_deathtypes = deathtypes;
	...
	level._arcadeMode_killColors = colors;
	...
	kill_bonuses[ "gun" ] = 0;
	kill_bonuses[ "none" ] = 0;
	level._arcadeMode_locationKillBonus = kill_bonuses;
	...
	level._arcadeMode_skillMultiplier = skill_multiplier;
	...
	*/
	...
	level._arcademode_kills_until_next_extra_life = 10;

	level._arcadeMode_extra_lives_range = extra_lives_range;

	// free points towards your first extra life
	extra_lives_base = [];
	for ( i = 0; i < 4; i++ )
		extra_lives_base[ i ] = extra_lives_range[ i ] * 0.15;
	level._arcadeMode_extra_lives_base = extra_lives_base;
}
...
new_ending_hud( align, fade_in_time, x_off, y_off )
{
	...
	hud_score.alpha = 1;
	hud_score.hidewheninmenu = true;
	hud_score.sort = level._arcadeMode_hud_sort + 10;
	return hud_score;
}
...
arcadeMode_add_points( origin, kill, type, value )
{
	...
	if ( isdefined( level._arcadeMode_deathtypes[ type ] ) )
		type = level._arcadeMode_deathtypes[ type ];

	value = int( value );
	value = round_up_to_five( value );

	value *= level._arcadeMode_kill_streak_current_multiplier;
	...
	if ( kill )
	{
		...
		color = level._arcadeMode_killColors[ type ];
...
//			level.player thread play_sound_in_space( "arcademode_kill", level.player geteye() );
	...
	level._player pointPulse( value );
}


arcademode_add_point_towards_extra_life()
{
	level._arcademode_kills_until_next_extra_life -= 1;
	if ( level._arcademode_kills_until_next_extra_life > 0 )
		return;

//	if ( score > level.arcademode_next_extra_life ) // getdvarint( "arcademode_next_extra_life" ) )

	level._arcademode_rewarded_lives++ ;

//		thread extra_lives_display();
	lives = getdvarint( "arcademode_lives" );
	lives++ ;
	if ( lives >= level._arcadeMode_maxlives )
	{
		// lives are capped at 10
		lives = level._arcadeMode_maxlives;
	}
	else
	{
		setdvar( "arcademode_lives", lives );
	...
	level._arcademode_kills_until_next_extra_life = level._arcadeMode_extra_lives_range[ level._gameskill ];
...
arcademode_set_origin_in_radius()
{
	r = 60;
	theta = 90;
	if ( level._player.pointPulseIndex > 0 )
	{
		if ( level._player.pointPulseIndex == 1 )
		{
			side = randomint( 1 );
			level._player.thirdPointPulseSide = 1 - side;
			if ( side )
				theta = 45;
			else
				theta = 135;
		}
		else if ( level._player.pointPulseIndex == 2 )
		{
			side = level._player.thirdPointPulseSide;
			if ( side )
				theta = 45;
			else
				theta = 135;
		}
		else if ( level._player.pointPulseIndex <= 4 )
		{
			theta = randomfloatrange( 0, 180 );
			r = randomfloatrange( 60, 120 );
		}
		else if ( level._player.pointPulseIndex <= 8 )
		{
			theta = randomfloatrange( 0, 180 );
...
pointPulse( amount )
{
	...
	if ( !isdefined( level._player.pointPulseCount ) )
	{
		level._player.pointPulseCount = 0;
		level._player.pointPulseIndex = 0;
	}
	...
	arcadeMode_kill_streak_current_multiplier = level._arcadeMode_kill_streak_current_multiplier;// store to local cause of the wait below

	level._player.pointPulseCount++ ;
	level._player.pointPulseIndex++ ;
	...
	else
	{
		...
		hud_pointpulse.glowColor = level._color_cool_green_glow;
...
//	hud_pointpulse setValue( amount );
	...
	level._player.pointPulseCount -- ;
	if ( level._player.pointPulseCount == 0 )
		level._player.pointPulseIndex = 0;
...
{
	// mod = method of death
	for ( i = 0; i < level._arcadeMode_multiKills[ mod ].size; i++ )
	{
		arcadeMode_add_points_for_individual_kill( level._arcadeMode_multiKills[ mod ][ i ], mod, level._arcadeMode_multiKills[ mod ].size );
	}
}
...
arcadeMode_add_points_for_individual_kill( mod_array, mod, kills )
{
	...
	if ( mod_array[ "type" ] != "melee" )
		base_points = level._arcadeMode_killBase + level._arcadeMode_locationKillBonus[ mod_array[ "damage_location" ] ] + level._arcadeMode_weaponBonus[ mod_array[ "type" ] ];
	else
		base_points = level._arcadeMode_killBase + level._arcadeMode_weaponBonus[ mod_array[ "type" ] ];
...
player_kill( type, location, point )
{
	...
	death_type = level._arcadeMode_deathtypes[ type ];
	if ( !isdefined( death_type ) )
	{
		// no location bonus for melee
		base_points = level._arcadeMode_killBase;
	...
	level._arcadeMode_multiKills[ death_type ][ level._arcadeMode_multiKills[ death_type ].size ] = array;
}

player_damage( type, location, point )
{
	thread arcadeMode_add_points( point, false, type, level._arcadeMode_damageBase );
}

player_damage_ads( type, location, point )
{
	thread arcadeMode_add_points( point, false, type, level._arcadeMode_damageBase * 1.25 );
}
...
{
	level._arcademode_hud_total_scores = [];

	for ( i = 0; i < level._arcademode_hud_digits; i++ )
	{
		hud_score = get_hud_score();
		level._arcademode_hud_total_scores[ level._arcademode_hud_total_scores.size ] = hud_score;
...
{
	hud_draw_score_for_elements( score, level._arcademode_hud_total_scores );
}
...
arcadeMode_ends()
{
	...
	stop_time = 0;
	if ( isdefined( 	level._arcademode_stoptime ) )
	{
		// the amount of time that the timer was stopped, at the end of the level
		stop_time = gettime() - level._arcademode_stoptime;
		stop_time *= 0.001;
	}

	time_elapsed = gettime() - level._arcdeMode_startTime;
	time_elapsed *= 0.001;
	time_elapsed -= stop_time;

	timer = level._arcademode_time - time_elapsed;
	timer = int( timer );
	if ( timer == 0 )
		timer++ ;
	...
	level._mission_failed_disabled = true;
	...
	else
	{
		...
		hud_end_reason settext( level._arcadeMode_failureString );
	}
	...
	for ( i = 0; i < level._arcadeMode_maxlives; i++ )
	{
		level._arcadeMode_lives_hud[ i ] destroy();
	}
	...
	for ( i = 0; i < level._arcadeMode_maxlives; i++ )
	{
		arcademode_add_life( i, -135 + offset, livesYpos, -30, 96, level._arcadeMode_hud_sort + 10 );
	}

	lives = getdvarint( "arcademode_lives" );
	rewarded_lives = level._arcademode_rewarded_lives;
...
//		hud_total_score_points setvalue( combined_score );
	...
	for ( i = 0; i < level._arcademode_hud_digits; i++ )
	{
		hud_score = level._arcademode_hud_scores[ i ];
		hud_score.x = i * - 30 + -150 + offset;
...
//	hud_score setvalue( 0 );
	...
	level._arcademode_hud_timer_minutes_tens = new_ending_hud( "right", fade_in_time, -265 + offset, timerYpos );
	level._arcademode_hud_timer_minutes_ones = new_ending_hud( "right", fade_in_time, -235 + offset, timerYpos );

	hud_timer_colon = new_ending_hud( "right", fade_in_time, -215 + offset, timerYpos - 5 );
	// :
	hud_timer_colon settext( &"SCRIPT_COLON" );

	level._arcademode_hud_timer_seconds_tens = new_ending_hud( "right", fade_in_time, -180 + offset, timerYpos );
	level._arcademode_hud_timer_seconds_ones = new_ending_hud( "right", fade_in_time, -150 + offset, timerYpos );
	...
	for ( ;; )
	{
		...
		*/
		...
		if ( soundCount <= 0 )
		{
			level._player thread play_sound_in_space( "bullet_ap_dirt", level._player geteye() );
	...
	if ( level._arcadeMode_success )
	{
		...
		final_time_bonus = ceil( base_time_bonus / level._arcademode_difficultyTimerScale );
...
//			hud_score setvalue( score );
			...
			if ( soundCount <= 0 )
			{
				level._player thread play_sound_in_space( "bullet_ap_metal", level._player geteye() );
				soundCount = 3;
			}
			wait( 0.05 );
		}
...
//				hud_total_score_points setvalue( combined_score );
			...
			score += boost;
			level._player thread play_sound_in_space( "mortar_explosion", level._player geteye() );
...
//			hud_score setvalue( int( score ) );
		...
		if ( getdvarint( "arcademode_died" ) != 1 && level._gameskill >= 2 )
		{
			boost = int( score );
			// ZERO DEATHS BONUS x2
	...
	else
	{
		...
		level._arcademode_hud_timer_seconds_ones SetPulseFX( 0, 0, 1000 );
		level._arcademode_hud_timer_seconds_tens SetPulseFX( 0, 0, 1000 );
		level._arcademode_hud_timer_minutes_ones SetPulseFX( 0, 0, 1000 );
		level._arcademode_hud_timer_minutes_tens SetPulseFX( 0, 0, 1000 );
	...
	skillmult = level._arcadeMode_skillMultiplier[ level._gameskill ];
	if ( skillmult > 1 )
	{
		if ( skillmult == 1.5 )
		{
	...
	if ( getdvar( "arcademode_full" ) == "1" )
	{
		...
		level._player UploadScore( "LB_FULL", getdvarint( dvar ) );
	}
	else
	{
		levelScoreIndices = [];
		...
		index = levelScoreIndices[ level._script ];
		...
		if ( isDefined( index ) )
		{
			...
			level._player UploadScore( levelLeaderboards[ level._script ], getdvarint( dvar ) );
		}
	}
	...
	if ( gotNewHighScore )
	{
		if ( !level._arcadeMode_success )// otherwise, UpdateGamerProfile() will be called from _endmission::_nextmission()
			UpdateGamerProfile();
	...
	if ( getdvar( "arcademode_full" ) == "1" )
	{
		...
		for ( i = 0; i < level._arcademode_hud_digits; i++ )
		{
			hud_score = level._arcademode_hud_total_scores[ i ];
			hud_score SetPulseFX( 0, 0, 1000 );
		}
	}
	...
	if ( getdvar( "arcademode_full" ) == "1" )
	{
		logString( "ArcadeMode Score: " + score + ", mission: " + level._script + ", gameskill: " + level._gameskill + ", total: " + combined_score );
	}
	else
	{
		logString( "ArcadeMode Score: " + score + ", mission: " + level._script + ", gameskill: " + level._gameskill );
	}
...
//			hud_total_score_points setvalue( combined_score );
		...
		if ( soundCount <= 0 )
		{
			level._player thread play_sound_in_space( sound, level._player geteye() );
...
//	overlay.foreground = true;
	overlay.sort = level._arcadeMode_hud_sort + 5;
	overlay.alpha = 0;
...
player_invul_forever()
{
	for ( ;; )
	{
		...
		level._player.deathInvulnerableTime = 70000;
...
ending_set_time( minutes, seconds )
{
	...
	level._arcademode_hud_timer_seconds_ones setvalue( seconds );
	level._arcademode_hud_timer_seconds_tens setvalue( seconds_tens );

	level._arcademode_hud_timer_minutes_ones setvalue( minutes );
	level._arcademode_hud_timer_minutes_tens setvalue( minutes_tens );
}
...
{
	for ( i = 0; i < level._arcadeMode_checkpoint_dvars.size; i++ )
	{
		if ( level._arcadeMode_checkpoint_dvars[ i ] == unique_id )
			return i;
	}
...
arcadeMode_init_kill_streak_colors()
{
	level._arcadeMode_streak_color = [];
	level._arcadeMode_streak_glow = [];

	level._arcadeMode_streak_color[ level._arcadeMode_streak_color.size ] = level._color_cool_green;// 1
	level._arcadeMode_streak_color[ level._arcadeMode_streak_color.size ] = ( 0.8, 0.8, 2.0 );// 2
	level._arcadeMode_streak_color[ level._arcadeMode_streak_color.size ] = ( 2.0, 0.8, 0.0 );// 4
	level._arcadeMode_streak_color[ level._arcadeMode_streak_color.size ] = ( 0.5, 2.0, 2.0 );// 3
	level._arcadeMode_streak_color[ level._arcadeMode_streak_color.size ] = ( 2.0, 0.5, 2.0 );// 6
	level._arcadeMode_streak_color[ level._arcadeMode_streak_color.size ] = ( 0.3, 0.3, 2.0 );// 7
	level._arcadeMode_streak_color[ level._arcadeMode_streak_color.size ] = ( 2.0, 2.0, 0.5 );// 5
	level._arcadeMode_streak_color[ level._arcadeMode_streak_color.size ] = ( 2.0, 2.0, 2.0 );// 8

	for ( i = 0; i < level._arcadeMode_streak_color.size; i++ )
	{
		level._arcadeMode_streak_glow[ i ] = ( level._arcadeMode_streak_color[ i ][ 0 ] * 0.35, level._arcadeMode_streak_color[ i ][ 1 ] * 0.35, level._arcadeMode_streak_color[ i ][ 2 ] * 0.35 );
	}

	// this one has a custom glow color so might as well use it
	level._arcadeMode_streak_color[ 0 ] = level._color_cool_green_glow;
}

arcadeMode_killstreak_complete_display()
{
	if ( level._arcadeMode_kill_streak_current_multiplier == 1 )
		return;
	...
	if ( level._arcadeMode_kill_streak_current_multiplier >= 8 )
	{
		level._player thread play_sound_in_space( "arcademode_kill_streak_won", level._player geteye() );
		// -STREAK COMPLETE-
		kill_streak settext( &"SCRIPT_STREAK_COMPLETE" );
	}
	else
	{
		level._player thread play_sound_in_space( "arcademode_kill_streak_lost", level._player geteye() );
		// STREAK BONUS LOST
...
{
	if ( isdefined( level._arcadeMode_streak_hud ) )
	{
		level._arcadeMode_streak_hud destroy();
		level._arcadeMode_streak_hud = undefined;
		level._arcadeMode_streak_hud_shadow destroy();
	}

	level notify( "arcademode_stop_kill_streak_art" );
	for ( i = 0; i < level._arcadeMode_kills_hud.size; i++ )
	{
		level._arcadeMode_kills_hud[ i ] destroy();
	}
	level._arcadeMode_kills_hud = [];
}
...
//	hud_mult.glowColor = level.color_cool_green_glow;
	hud_mult.color = level._arcadeMode_streak_color[ level._arcadeMode_kill_streak_current_multiplier - 1 ];
	hud_mult.glowColor = level._arcadeMode_streak_glow[ level._arcadeMode_kill_streak_current_multiplier - 1 ];

	hud_mult.sort = level._arcadeMode_hud_sort;

	// x
	hud_mult.label = &"SCRIPT_X";
	hud_mult setvalue( level._arcadeMode_kill_streak_current_multiplier );
...
arcademode_draw_multiplier()
{
	...
	level._arcademode_hud_streak = hud_mult;
	level waittill( "arcademode_stop_kill_streak" );
	hud_mult SetPulseFX( 0, 0, 1000 );
	wait( 1 );
	hud_mult destroy();
	level._arcademode_hud_streak = undefined;
}
