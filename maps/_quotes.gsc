...
//	deadquotes[ deadquotes.size ]  = "@DEADQUOTE_SO_ICON_OBJ_OFFSCREEN";
	...
	{
		deadquotes[ deadquotes.size ]  = "@DEADQUOTE_SO_FIND_FRIEND_COOP";
	}
...
should_use_custom_deadquotes()
{
	...
	if ( level._so_deadquotes.size <= 0 )
		return false;
		
	assertex( isdefined( level._so_deadquotes_chance ), "level.so_deadquotes had contents, but level.so_deadquote_chance was undefined." );	

	// Set level.so_deadquotes_chance to 1.0 to guarantee the contents will be used.
	return ( level._so_deadquotes_chance >= randomfloat( 1.0 ) );
}