...
// 	wait( 0.05 );// begone infinite hangs
	level._arrays_of_colorCoded_nodes = [];
	level._arrays_of_colorCoded_nodes[ "axis" ] = [];
	level._arrays_of_colorCoded_nodes[ "allies" ] = [];

	level._arrays_of_colorCoded_ares_nodes = [];
	level._arrays_of_colorCoded_ares_nodes[ "axis" ] = [];
	level._arrays_of_colorCoded_ares_nodes[ "allies" ] = [];

	level._arrays_of_colorCoded_volumes = [];
	level._arrays_of_colorCoded_volumes[ "axis" ] = [];
	level._arrays_of_colorCoded_volumes[ "allies" ] = [];
	triggers = [];
	...
	level._color_teams = [];
	level._color_teams[ "allies" ] = "allies";
	level._color_teams[ "axis" ] = "axis";
	level._color_teams[ "team3" ] = "axis";
	level._color_teams[ "neutral" ] = "neutral";
	...
	// go through all structs to find ares color nodes
	foreach ( struct in level._struct )
	{
		if ( isdefined( struct.script_color_allies ) )
		{
			struct add_struct_to_global_ares_arrays( struct.script_color_allies, "allies" );
		}
		if ( isdefined( struct.script_color_axis ) )
		{
			struct add_struct_to_global_ares_arrays( struct.script_color_axis, "axis" );
		}
	}
	...
	foreach ( trigger in triggers )
	{
	 ...
	 /#
		level._colorNodes_debug_array = [];
		level._colorNodes_debug_array[ "allies" ] = [];
		level._colorNodes_debug_array[ "axis" ] = [];

// 		level.colorNodes_debug_ai = [];
	#/

	level._color_node_type_function = [];
	...
	level._colorList = [];
	level._colorList[ level._colorList.size ] = "r";
	level._colorList[ level._colorList.size ] = "b";
	level._colorList[ level._colorList.size ] = "y";
	level._colorList[ level._colorList.size ] = "c";
	level._colorList[ level._colorList.size ] = "g";
	level._colorList[ level._colorList.size ] = "p";
	level._colorList[ level._colorList.size ] = "o";

	level._colorCheckList[ "red" ] 	 = "r";
	level._colorCheckList[ "r" ] 		 = "r";
	level._colorCheckList[ "blue" ] 	 = "b";
	level._colorCheckList[ "b" ] 		 = "b";
	level._colorCheckList[ "yellow" ] 	 = "y";
	level._colorCheckList[ "y" ] 		 = "y";
	level._colorCheckList[ "cyan" ] 	 = "c";
	level._colorCheckList[ "c" ] 		 = "c";
	level._colorCheckList[ "green" ] 	 = "g";
	level._colorCheckList[ "g" ] 		 = "g";
	level._colorCheckList[ "purple" ] 	 = "p";
	level._colorCheckList[ "p" ] 		 = "p";
	level._colorCheckList[ "orange" ] 	 = "o";
	level._colorCheckList[ "o" ] 		 = "o";

	level._currentColorForced = [];
	level._currentColorForced[ "allies" ] = [];
	level._currentColorForced[ "axis" ] = [];

	level._lastColorForced = [];
	level._lastColorForced[ "allies" ] = [];
	level._lastColorForced[ "axis" ] = [];

	foreach ( color in level._colorList )
	{
		level._arrays_of_colorForced_ai[ "allies" ][ color ] = [];
		level._arrays_of_colorForced_ai[ "axis" ][ color ] = [];
		level._currentColorForced[ "allies" ][ color ] = undefined;
		level._currentColorForced[ "axis" ][ color ] = undefined;
	}
...
convert_color_to_short_string()
{
	...
	self.script_forceColor = level._colorCheckList[ self.script_forceColor ];
}
...
ai_picks_destination( currentColorCode )
{
	if ( isdefined( self.script_forcecolor ) )
	{
		...
		*/ 
		...
		level._arrays_of_colorForced_ai[ self get_team() ][ color ] = array_add( level._arrays_of_colorForced_ai[ self get_team() ][ color ], self );
...
goto_current_ColorIndex()
{
	...
	nodes = level._arrays_of_colorCoded_nodes[ self get_team() ][ self.currentColorCode ];
	...
	if ( !isdefined( nodes ) )
	{
		volume = level._arrays_of_colorCoded_volumes[ self get_team() ][ self.currentColorCode ];
...
get_colorcodes_from_trigger( color_team, team )
{
	...
	foreach ( colorCode in colorCodes )
	{
		...
		assertEx( isdefined( color ), "Strange color order issued: " + colorCode );
...
colorCode_is_used_in_map( team, colorCode )
{
	...
	if ( isdefined( level._arrays_of_colorCoded_nodes[ team ][ colorCode ] ) )
		return true;

	// what about a goalvolume
	return isdefined( level._arrays_of_colorCoded_volumes[ team ][ colorCode ] );
}
...
// 		waittillframeend; 
	...
	for ( i = 0; i < colorCodes.size; i++ )
	{
		if ( !isdefined( level._arrays_of_colorCoded_spawners[ team ][ colorCodes[ i ] ] ) )
			continue;

		// remove deleted spawners
		level._arrays_of_colorCoded_spawners[ team ][ colorCodes[ i ] ] = array_removeUndefined( level._arrays_of_colorCoded_spawners[ team ][ colorCodes[ i ] ] );

		// set the .currentColorCode on each appropriate spawner
		for ( p = 0; p < level._arrays_of_colorCoded_spawners[ team ][ colorCodes[ i ] ].size; p++ )
			level._arrays_of_colorCoded_spawners[ team ][ colorCodes[ i ] ][ p ].currentColorCode = colorCodes[ i ];
	}
	...
	foreach ( color in colors )
	{
		...
		level._lastColorForced[ team ][ color ] = level._currentColorForced[ team ][ color ];

		// set the destination of the color forced spawners
		level._currentColorForced[ team ][ color ] = colorCodesByColorIndex[ color ];


		/#
		color_forced = level._currentColorForced[ team ][ color ];
		color_defined = isdefined( level._arrays_of_colorCoded_nodes[ team ][ color_forced ] ) || isdefined( level._arrays_of_colorCoded_volumes[ team ][ color_forced ] );
	...
	for ( i = 0; i < colorCodes.size; i++ )
	{
		...
		if ( !isdefined( level._arrays_of_colorCoded_ai[ team ][ colorCode ] ) )
			continue;
...
{
	if ( !isdefined( level._lastColorForced[ team ][ color ] ) )
		return false;

	return level._lastColorForced[ team ][ color ] == level._currentColorForced[ team ][ color ];
}
...
{
	nodes = level._arrays_of_colorCoded_nodes[ team ][ colorCode ];
	...
	lastColorForced_exists = isdefined( level._lastColorForced[ team ][ color ] );

	// fills ent.path_nodes or .cover_nodes depending on node type	
	for ( i = 0 ; i < nodes.size; i++ )
	{
		node = nodes[ i ];
		ent [[ level._color_node_type_function[ node.type ][ lastColorForced_exists ][ team ] ]]( node, level._lastColorForced[ team ][ color ] );
	}
	...
	level._arrays_of_colorCoded_nodes[ team ][ colorCode ] = nodes;
}

get_prioritized_colorCoded_nodes( team, colorCode, color )
{
	return level._arrays_of_colorCoded_nodes[ team ][ colorCode ];
}

get_colorCoded_ares_nodes( team, colorCode, color )
{
	return level._arrays_of_colorCoded_ares_nodes[ team ][ colorCode ];
}

get_colorCoded_volume( team, colorCode )
{
	return level._arrays_of_colorCoded_volumes[ team ][ colorCode ];
}

issue_leave_node_order_to_ai_and_get_ai( colorCode, color, team )
{
	// remove dead from this specific colorCode
	level._arrays_of_colorCoded_ai[ team ][ colorCode ] = array_removeDead( level._arrays_of_colorCoded_ai[ team ][ colorCode ] );
	ai = level._arrays_of_colorCoded_ai[ team ][ colorCode ];
	ai = array_combine( ai, level._arrays_of_colorForced_ai[ team ][ color ] );
	newArray = [];
	foreach ( guy in ai )
	{
...
issue_color_order_to_ai( colorCode, color, team, ai_array )
{
	original_ai_array = ai_array;

	/# level._colorNodes_debug_array[ team ][ colorCode ] = undefined; #/

	nodes = [];
	ares_nodes = [];

	if ( isdefined( level._arrays_of_colorCoded_ares_nodes[ team ][ colorCode ] ) )
	{
		ares_nodes = get_colorCoded_ares_nodes( team, colorCode, color );
	}
	...
	if ( isdefined( level._arrays_of_colorCoded_nodes[ team ][ colorCode ] ) )
	{
		...
		nodes = get_prioritized_colorCoded_nodes( team, colorCode, color );
		 ...
		 /#
		level._colorNodes_debug_array[ team ][ colorCode ] = nodes;
		#/
	}
	else
	{
		...
		array_thread( ai_array, ::send_ai_to_colorVolume, volume, colorCode );
	}

	 /#
	if ( nodes.size < ai_array.size )
		println( "^3Warning, ColorNumber system tried to make " + ai_array.size + " AI go to " + nodes.size + " nodes." );
	#/

	counter = 0;
	ai_count = ai_array.size;

	// Split up ai into ares and non-ares
	ai_index = 0;
	ares_array = [];
	while ( ai_index < ai_array.size )
	{
		current_ai = ai_array[ai_index];
		if ( current_ai.subclass == "ares" )
		{
			ares_array[ ares_array.size ] = current_ai;
			ai_array = array_remove( ai_array, current_ai );
			continue;
		}
		ai_index++;
	}

	// Issue order to all ares first
	ai_index = 0;
	node_index = 0;

	for ( i = 0; i < ares_nodes.size; i++ )
	{
		node = ares_nodes[ i ];
		...
		closestAI = getclosest( node.origin, ares_array );
		assert( isalive( closestAI ) );
		ares_array = array_remove( ares_array, closestAI );

// 			println( "sent ai to node" );
		closestAI ares_take_color_node( node, colorCode, self, counter );
		counter++;
	...
	for ( i = 0; i < nodes.size; i++ )
	{
		...
		closestAI = getclosest( node.origin, ai_array );
		assert( isalive( closestAI ) );
		ai_array = array_remove( ai_array, closestAI );
...
ares_take_color_node( node, colorCode, trigger, counter )
{
	self notify( "stop_color_move" );
	self.currentColorCode = colorCode;
	self thread process_color_order_to_ares( node, trigger, counter );
}
...
player_color_node()
{
	...
	for ( ;; )
	{
		...
		olduser = level._player.node.color_user;

		playerNode = level._player.node;
		playerNode.color_user = level._player;
...
*/ 
		...
		for ( ;; )
		{
			if ( !isdefined( level._player.node ) )
				break;
			if ( level._player.node != playerNode )
				break;
			wait( 0.05 );
		}
...
color_node_finds_user_for_colorCode( colorCode, team )
{
	...
	if ( !isdefined( level._currentColorForced[ team ][ color ] ) )
	{
		// AI of this color are not assigned to a colornode currently
		return;
	}

	if ( level._currentColorForced[ team ][ color ] != colorCode )
	{
...
set_goal_and_volume( node )
{
	...
	{
		if( isdefined( level._fixednodesaferadius_default ) )
			self.fixedNodeSafeRadius = level._fixednodesaferadius_default;
		else
...
process_color_order_to_ares( struct, trigger, counter )
{
	thread decrementColorUsers( struct );
...
// 	waitSpread( delay_size * 0.5 );// they spread out the time they leave based on how many guys it is
	...
	self.color_ordered_node_assignment = struct;

	// tagJW<NOTE>: Color Ares currently do not support clearing node when player claims it
	/*for ( ;; )
	{
...
get_best_available_colored_node()
{
	...
	colorCode = level._currentColorForced[ self get_team() ][ self.script_forceColor ];
...
colorIsLegit( color )
{
	for ( i = 0; i < level._colorList.size; i++ )
	{
		if ( color == level._colorList[ i ] )
			return true;
	}
	return false;
}
...
add_volume_to_global_arrays( colorCode_string, team )
{
	...
	foreach ( colorCode in colorCodes )
	{
		assert( !isdefined( level._arrays_of_colorCoded_volumes[ team ][ colorCode ] ), "Multiple info_volumes exist with color code " + colorCode );

		level._arrays_of_colorCoded_volumes[ team ][ colorCode ] = self;
		level._arrays_of_colorCoded_ai[ team ][ colorCode ] = [];
		level._arrays_of_colorCoded_spawners[ team ][ colorCode ] = [];
	}
}

add_struct_to_global_ares_arrays( colorCode_string, team )
{
	...
	foreach ( colorCode in colorCodes )
	{
		if ( isdefined( level._arrays_of_colorCoded_ares_nodes[ team ] ) && isdefined( level._arrays_of_colorCoded_ares_nodes[ team ][ colorCode ] ) )
		{
			// array already exists so add this color coded node to that color code array.
			level._arrays_of_colorCoded_ares_nodes[ team ][ colorCode ] = array_add( level._arrays_of_colorCoded_ares_nodes[ team ][ colorCode ], self );
			continue;
		}

		// array doesn't exist so we have to initialize all the variables related to this color coding.
		level._arrays_of_colorCoded_ares_nodes[ team ][ colorCode ][ 0 ] = self;
		//level._arrays_of_colorCoded_ai[ team ][ colorCode ] = [];
		//level._arrays_of_colorCoded_spawners[ team ][ colorCode ] = [];
	}
}
...
// 	assertEx( self.radius > 0, "Node " + self.type + " at origin " + self.origin + " does not have a radius set in Radiant." );
	...
	foreach ( colorCode in colorCodes )
	{
		if ( isdefined( level._arrays_of_colorCoded_nodes[ team ] ) && isdefined( level._arrays_of_colorCoded_nodes[ team ][ colorCode ] ) )
		{
			// array already exists so add this color coded node to that color code array.
			level._arrays_of_colorCoded_nodes[ team ][ colorCode ] = array_add( level._arrays_of_colorCoded_nodes[ team ][ colorCode ], self );
			continue;
		}

		// array doesn't exist so we have to initialize all the variables related to this color coding.
		level._arrays_of_colorCoded_nodes[ team ][ colorCode ][ 0 ] = self;
		level._arrays_of_colorCoded_ai[ team ][ colorCode ] = [];
		level._arrays_of_colorCoded_spawners[ team ][ colorCode ] = [];
	}
}
...
removeSpawnerFromColorNumberArray()
{
	...
	for ( i = 0;i < colors.size;i++ )
		level._arrays_of_colorCoded_spawners[ team ][ colors[ i ] ] = array_remove( level._arrays_of_colorCoded_spawners[ team ][ colors[ i ] ], self );
}


add_cover_node( type )
{
	level._color_node_type_function[ type ][ true ][ "allies" ] = ::process_cover_node_with_last_in_mind_allies;
	level._color_node_type_function[ type ][ true ][ "axis" ] = ::process_cover_node_with_last_in_mind_axis;
	level._color_node_type_function[ type ][ false ][ "allies" ] = ::process_cover_node;
	level._color_node_type_function[ type ][ false ][ "axis" ] = ::process_cover_node;
}

add_path_node( type )
{
	level._color_node_type_function[ type ][ true ][ "allies" ] = ::process_path_node;
	level._color_node_type_function[ type ][ false ][ "allies" ] = ::process_path_node;
	level._color_node_type_function[ type ][ true ][ "axis" ] = ::process_path_node;
	level._color_node_type_function[ type ][ false ][ "axis" ] = ::process_path_node;
}
...
colorNode_spawn_reinforcement( classname, fromColor )
{
	...
	if ( isdefined( level._friendly_startup_thread ) )
		reinforcement thread [[ level._friendly_startup_thread ]]();
...
colorNode_replace_on_death()
{
	...
	color_order = level._current_color_order;
	...
	for ( ;; )
	{
		...
		if ( !isdefined( level._color_doesnt_care_about_heroes ) )
			correct_colored_friendlies = remove_heroes_from_array( correct_colored_friendlies );
		if ( !isdefined( level._color_doesnt_care_about_classname ) )
		...
		correct_colored_guy = getclosest( level._player.origin, correct_colored_friendlies );
		...
		if ( isdefined( level._friendly_promotion_thread ) )
		{
			correct_colored_guy [[ level._friendly_promotion_thread ]]( color );
		}
...
{
	level._friendly_respawn_vision_checker_thread = true;
	...
	for ( ;; )
	{
		...
		if ( !isdefined( level._respawn_spawner_org ) )
			continue;

		difference_vec = level._player.origin - level._respawn_spawner_org;
	    ...
	    forward = anglesToForward( ( 0, level._player getplayerangles()[ 1 ], 0 ) );
...
{
	if ( isdefined( level._respawn_friendlies_force_vision_check ) )
		return false;
...
spawn_hidden_reinforcement( classname, fromColor )
{
	...
	for ( ;; )
	{
		if ( !respawn_friendlies_without_vision_check() )
		{
			if ( !isdefined( level._friendly_respawn_vision_checker_thread ) )
		...
		spawner.origin = level._respawn_spawner_org;
//		assertEx( spawner.spawnflags & 4, "Friendly color spawner with export " + spawner.export + " does not have UNDELETEABLE checkbox." );
	...
	for ( ;; )
	{
		...
		if ( get_color_from_order( fromColor, level._current_color_order ) == "none" )
			break;
		fromColor = level._current_color_order[ fromColor ];
	}
	if ( isdefined( fromColor ) )
...
get_team( team )
{
	if ( isdefined( self.team ) && !isdefined( team ) )
		team = self.team;
	// tmp stuff for making colors work on team 3.
	return( level._color_teams[ team ] );
}
