...
hurtgen_style()
{
	...
	while ( 1 )
	{
		...
		for ( i = 0;i < mortars.size;i++ )
		{
			...
			d = distance( level._player getorigin(), mortars[ c ].origin );
			d2 = undefined;
			if ( isdefined( level._foley ) )
				d2 = distance( level._foley.origin, mortars[ c ].origin );
			else
				d2 = 360;
...
railyard_style( fRandomtime, iMaxRange, iMinRange, iBlastRadius, iDamageMax, iDamageMin, fQuakepower, iQuaketime, iQuakeradius, targetsUsed, seedtime )
{
	...
	if ( !isdefined( level._iStopBarrage ) )
		level._iStopBarrage = 0;
	...
	if ( isdefined( level._mortar_notify ) )
		level waittill( level._mortar_notify );

	for ( ;; )
	{
		if ( level._iStopBarrage != 0 )
			wait 1;
		while ( level._iStopBarrage == 0 )
		{
...
script_mortargroup_style()
{
	...
	for ( i = 0;i < models.size;i++ )
		...
		{
			if ( !isdefined( level._mortars[ models[ i ].script_mortargroup ] ) )
				level._mortars[ models[ i ].script_mortargroup ] = [];
			...
			level._mortars[ models[ i ].script_mortargroup ]
			[ level._mortars[ models[ i ].script_mortargroup ].size ] = mortar;
			models[ i ] delete();
...
//		mortars[i] setup_mortar_terrain(); 		// this was commented out going to run it and find out just why
	...
	if ( !( isdefined( level._mortar ) ) )
		level._mortar = loadfx( "explosions/artilleryExp_dirt_brown" );
	...
	for ( i = 0;i < triggers.size;i++ )
		...
		{
			if ( !isdefined( level._mortars[ triggers[ i ].script_mortargroup ] ) )
				level._mortars[ triggers[ i ].script_mortargroup ] = [];
	...
	while ( 1 )
	{
		...
		level._mortarzone = mortartrig.script_mortargroup;
...
script_mortargroup_mortarzone()
{
	...
	while ( ( level._mortars[ self.script_mortargroup ].size > 0 && ( level._mortarzone == self.script_mortargroup ) ) || timed )
	{
		if ( customdelay )
		{
		...
		mortarsinfront = [];
		pick = randomint( level._mortars[ self.script_mortargroup ].size );
		if ( randomint( 100 ) < 75 )
		{
			playerforward = anglestoforward( level._player.angles );
			points = [];
			for ( i = 0;i < level._mortars[ self.script_mortargroup ].size;i++ )
			{
				if ( mortar_radius > 0 && distance( level._player.origin, level._mortars[ self.script_mortargroup ][ i ].origin ) > mortar_radius )
					continue;
				if ( is_lastblast( level._mortars[ self.script_mortargroup ][ i ], lastblast ) )
					continue;
				normalvec = vectornormalize( level._mortars[ self.script_mortargroup ][ i ].origin - level._player.origin );
		...
		if ( lastblast.size > 3 )
			lastblast = [];
		lastblast[ lastblast.size ] = level._mortars[ self.script_mortargroup ][ pick ];
		level._mortars[ self.script_mortargroup ][ pick ] thread script_mortargroup_domortar();
...
{
	if ( isdefined( self.targetname ) && isdefined( level._mortarthread[ self.targetname ] ) )
		level thread [[ level._mortarthread[ self.targetname ] ]]( self );
	else
...
script_mortargroup_mortar_group()
{
	while ( 1 )
	{
		...
		if ( isdefined( level._mortarzone ) && level._mortarzone == self.script_mortargroup )
			continue;
...
trigger_targeted()
{
	...
	level._mortartrigger = getentarray( "mortartrigger", "targetname" );
	level._mortars = getentarray( "script_origin", "classname" );

	for ( i = 0;i < level._mortars.size;i++ )
	{
		if ( isdefined( level._mortars[ i ].script_mortargroup ) )
		{
			level._mortars[ i ] setup_mortar_terrain();
		}
	}
	...
	for ( i = 0;i < level._mortartrigger.size;i++ )
	{
...
{
	targeted_mortars = getentarray( level._mortartrigger[ num ].target, "targetname" );

	while ( 1 )
	{
		if ( level._player istouching( level._mortartrigger[ num ] ) )
		{
			r = randomint( targeted_mortars.size );
			while ( r == level._lastmortar )
			{
				r = randomint( targeted_mortars.size );
				wait .1;
			}
...
bunker_style_mortar_think( mortar_bunker_array, structs )
{
	min = undefined;
	max = undefined;
	if ( isdefined( level._mortarMinInterval ) )
		min = level._mortarMinInterval;
	else
		min = 4;
	if ( isdefined( level._mortarMaxInterval ) )
		max = level._mortarMaxInterval;
	else
		max = 6;
...
bunker_style_mortar_activate( mortar_bunker_array, min, max, groupNum, structs )
{
	...
	{
		wait( 0.05 );
		sound_org = getclosest( level._player.origin, structs ); 
		if( !isdefined( level._mortarNoIncomingSound ) )
		{
		...
		if( !isdefined( level._mortarNoQuake ) )
		{
			if ( cointoss() )
...
bunker_style_mortar_explode( min, max )
{
	...
	-------------------------*/	
	if ( ( isdefined( level._mortarWithinFOV ) ) && ( self mortar_within_player_fov( level._mortarWithinFOV ) == false ) )
		return;
		
	/*-----------------------
	ONLY PLAY EFFECT/ACTIVATE TRIGGER IF WITHIN RADIUS
	-------------------------*/			
	if ( isdefined( level._mortar_min_dist ) )
		min_dist = level._mortar_min_dist;
	else
		min_dist = 1024;
	
	min_dist_squared = min_dist * min_dist;
	distSquared = distancesquared( level._player.origin, self.origin );
	...
	if ( ( isdefined( self.classname ) ) && ( self.classname == "trigger_radius") ) 
	{
		...
		if ( ( !level._player istouching( self ) ) && ( distance( level._player.origin, self.origin ) < level._mortarDamageTriggerDist ) )
		{
...
bog_style_mortar_activate( mortars, groupNum, min, max )
{
	...
	for ( ;; )
	{
		for ( ;; )
		{
			...
			d = distance( level._player.origin, mortars[ rand ].origin );
			if ( d < min_dist )
				continue;
				
			//don't trigger mortar in case we have friendlies we do not want to be hit by mortars
			if ( ( isdefined( level._mortarExcluders ) ) && ( level._mortarExcluders.size > 0 ) )
			{
				...
				if ( mortars_too_close( level._mortarExcluders, mortarDistanceTrigger ) )
					continue;
			} 

			// in case we need to see mortars in the distance
			if ( !isdefined( level._noMaxMortarDist ) && ( d > 1000 ) )
				continue;
			if ( ( isdefined( level._mortar_max_dist ) ) && ( d > level._mortar_max_dist ) )
				continue;
			//if we need to check player FOV
			if ( ( isdefined( level._mortarWithinFOV ) ) && ( mortars[ rand ] mortar_within_player_fov( level._mortarWithinFOV ) == false ) )
				continue;
			break;
		}
		if ( ( isdefined( level._noMortars ) ) && ( level._noMortars == true ) )
			return;
...
{
	foreach ( guy in level._mortarExcluders )
	{
		if ( !isalive( guy ) )
			continue;
		if ( !isdefined( guy ) )
...
mortar_within_player_fov( fov )
{
	...
	if ( isdefined( level._playerMortarFovOffset ) )
		playerMortarFovOffset = level._playerMortarFovOffset;

	bInFOV = within_fov( playerEye, level._player getPlayerAngles() + playerMortarFovOffset, self.origin, fov );
	return bInFOV;
}

bog_style_mortar_explode( instant, customExploSound )
{
	if ( !isdefined( level._mortarDamageRadius ) )
		level._mortarDamageRadius = 250;
	if ( !isdefined( instant ) )
		instant = false;

	self thread bog_style_mortar_cooldown();
	if ( !instant )
		self play_sound_in_space( level._scr_sound[ "mortar" ][ "incomming" ] );
	if ( isdefined( customExploSound ) )
		self thread play_sound_in_space( customExploSound );
	else
		self thread play_sound_in_space( level._scr_sound[ "mortar" ][ self.script_fxid ] );

	//dont play an effect if it's behind the player - trying to save on particle effects
	setplayerignoreradiusdamage( true );
	radiusDamage( self.origin, level._mortarDamageRadius, 150, 50 );
	setplayerignoreradiusdamage( false );
	playfx( level._effect[ "mortar" ][ self.script_fxid ], self.origin );

	if( isdefined( level._alwaysquake ) )
	{
		earthquake( 0.3, 1, level._player.origin, 2000 );
	}

	if ( getdvarint( "bog_camerashake" ) > 0 )
	{
		//if player is trying to snipe dont do view shake
		if ( ( level._player getCurrentWeapon() == "dragunov" ) && ( level._player playerADS() > 0.8 ) )
			return;
...
{
	if ( !isdefined( level._mortarDamageTriggerDist ) )
		level._mortarDamageTriggerDist = 512;
	if ( !isdefined( level._mortarWithinFOV ) )
		level._mortarWithinFOV = cos( 35 );
...
burnville_style_mortar()
{
	...
	while ( 1 )
	{
		if ( distance( level._player getorigin(), self.origin ) < 600 )
		{
	...
	while ( 1 )
	{
		if ( ( distance( level._player getorigin(), self.origin ) < 1200 ) &&
			( distance( level._player getorigin(), self.origin ) > 400 ) )
		{
			activate_mortar( undefined, undefined, undefined, undefined, undefined, undefined, false );
...
mortar_boom( origin, fPower, iTime, iRadius, effect, bIsstruct )
{
	...
	if ( isdefined( effect ) )
		...
		playfx( level._mortar, origin );

	earthquake( fPower, iTime, origin, iRadius );

	// Special Burnville Shell shocking
	if ( level._script != "burnville" )
		return;
	if ( isdefined( level._playerMortar ) )
		return;
	if ( distance( level._player.origin, origin ) > 300 )
		return;
	if ( level._script == "carchase" || level._script == "breakout" )
		return;

	level._playerMortar = true;
...
{
	if ( !isdefined( level._mortar_last_sound ) )
		level._mortar_last_sound = -1;

	soundnum = randomint( 3 ) + 1;
	while ( soundnum == level._mortar_last_sound )
		soundnum = randomint( 3 ) + 1;

	level._mortar_last_sound	 = soundnum;
...
{
	currenttime = gettime();
	if ( !isdefined( level._lastmortarincomingtime ) )
	{
		level._lastmortarincomingtime = currenttime;
	}
	else if ( ( currenttime - level._lastmortarincomingtime ) < 1000 )
	{
		wait 1;
		return;
	}
...
generic_style( strExplosion, fDelay, iBarrageSize, fBarrageDelay, iMinRange, iMaxRange, bTargetsUsed )
{
	...
	if ( isdefined( level._explosion_stopNotify ) && isdefined( level._explosion_stopNotify[ strExplosion ] ) )
		level endon( level._explosion_stopNotify[ strExplosion ] );

	// for backwards compatibility
	if ( !isdefined( level._bStopBarrage ) || !isdefined( level._bStopBarrage[ strExplosion ] ) )
		level._bStopBarrage[ strExplosion ] = false;
	...
	//// Start Wait
	if ( isdefined( level._explosion_startNotify ) && isdefined( level._explosion_startNotify[ strExplosion ] ) )
		level waittill( level._explosion_startNotify[ strExplosion ] );

	//// Main Loop
	while ( true )
	{
		while ( !level._bStopBarrage[ strExplosion ] )
		{
			for ( j = 0; j < iBarrageSize; j++ )
			{
				...
				for ( i = 0; i < aeExplosions.size; i++ )
				{
					...
					fDist = distance( level._player getorigin(), aeExplosions[ iCur ].origin );
	...
	//				if ( (fDist < iMaxRangeLocal) && (fDist > iMinRangeLocal))
				...
				iLastExplosion = -1;
				if ( isdefined( level._explosion_delay ) && isdefined( level._explosion_delay[ strExplosion ] ) )
					wait( level._explosion_delay[ strExplosion ] );
				else
					wait( randomfloat( fDelay ) + randomFloat( fDelay ) );
			}
			if ( isdefined( level._explosion_barrage_delay ) && isdefined( level._explosion_barrage_delay[ strExplosion ] ) )
				wait( level._explosion_barrage_delay[ strExplosion ] );
			else
...
explosion_activate( strExplosion, iBlastRadius, iDamageMin, iDamageMax, fQuakePower, iQuakeTime, iQuakeRadius )
{
	...
	fPreDist = undefined;
	eLocation = self;
	if ( isdefined( self.iMinRange ) && distance( level._player.origin, self.origin ) < self.iMinRange )
	{
...
explosion_boom( strExplosion, fPower, iTime, iRadius )
{
	...
	if ( distance( level._player.origin, explosion_origin ) > 300 )
		return;
	if ( level._script == "carchase" || level._script == "breakout" )
		return;
