...
main( vehicletype )
{
	...
	if ( !isdefined( vehicletype ) )
		return;
	level._vehicle_treads[ vehicletype ] = true;
	switch( vehicletype )
	{
		case "apache":
		case "cobra":
		...
		case "nx_miniuav_player":
			setallvehiclefx( vehicletype, "nx/treadfx/miniuav_dust_default" );
			break;
		case "nx_lunar_rover":
			setallvehiclefx( vehicletype, "nx/lunar/treadfx/nx_lunar_tread_rover_regolith" );
			break;
		case "nx_chinese_lgv":
			setallvehiclefx( vehicletype, "nx/lunar/treadfx/nx_lunar_tread_rover_regolith" );
			break;
		case "nx_chinese_skimmer":
			setallvehiclefx( vehicletype, "nx/lunar/treadfx/nx_lunar_tread_dust_default" );
			break;
		case "nx_chinese_ugv_cheapfx":		
			setallvehiclefx( vehicletype );
			break;
		case "nx_parachute": // No tread fx for nx_parachute or the cheapfx ugv currently
			level._vehicle_treads[ vehicletype ] = false;
			setallvehiclefx( vehicletype );
			break;
		default:// if the vehicle isn't in this list it will use these effects
			...
			//setvehiclefx( vehicletype, "dirt", "treadfx/tread_dust_default" );
