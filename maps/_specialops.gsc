...
specialops_init()
{
	if ( !isdefined( level._so_override ) )
		level._so_override = [];
	...
	level._no_friendly_fire_penalty = true;
...
//	PreCacheShader( "hud_star69icon" );
	...
	level._challenge_time_nudge = 30;	// Yellow warning at 30 seconds
	level._challenge_time_hurry = 10;	// Red Hurry Up at 15 seconds
	
	// Function to handle crushing players when inside of a clip.
	level._func_destructible_crush_player = ::so_crush_player;
	
	// Default friendly fire scaler.
	setsaveddvar( "g_friendlyfireDamageScale", 2 );
	
	if ( isdefined( level._so_compass_zoom ) )
	{
		compass_dist = 0;
		switch ( level._so_compass_zoom )
		{
	...
	level._so_deadquotes_chance = 0.5;	// 50/50 chance of using level specific deadquotes.
...
fade_challenge_in( wait_time, doDialogue )
{
	...
	alpha = 1;
	if ( isdefined( level._so_waiting_for_players_alpha ) )
		alpha = level._so_waiting_for_players_alpha;
...
enable_challenge_timer( start_flag, passed_flag, message )
{
	...
	{	
		...
		level._passed_flag = passed_flag;
	}
	
	if ( !isdefined( message ) )
		message = &"SPECIAL_OPS_TIME";

	if ( !isdefined( level._challenge_time_beep_start ) )
		level._challenge_time_beep_start = level._challenge_time_hurry;
	level._so_challenge_time_beep = level._challenge_time_beep_start + 1;
...
so_wait_for_players_ready()
{
	...
	if ( !isdefined( level._so_enable_wait_for_players ) )
		return;
		
	if ( !is_coop() || issplitscreen() )
		return;

	level._so_waiting_for_players = true;
	level._so_waiting_for_players_alpha = 0.85;

	level._player thread so_wait_for_player_ready( "special_op_p1ready", 2 );
	level._player2 thread so_wait_for_player_ready( "special_op_p2ready", 3.25 );

	screen_hold = create_client_overlay( "black", 1 );
	screen_hold fade_over_time( level._so_waiting_for_players_alpha, 1 );
	...
	level._player thread so_wait_for_player_ready_cleanup( hold_time );
	level._player2 thread so_wait_for_player_ready_cleanup( hold_time );

	wait hold_time;
	
	screen_hold Destroy();
	level._so_waiting_for_players = undefined;
}
...
attacker_is_p2( attacker )
{
	...
	return attacker == level._player2;
}
...
enable_escape_warning()
{
	...
	level._escape_warning_triggers = getentarray( "player_trying_to_escape", "script_noteworthy" );
	assertex( level._escape_warning_triggers.size > 0, "enable_escape_warning() requires at least one trigger with script_noteworthy = player_trying_to_escape" );
...
so_force_deadquote( quote, icon_dvar )
{
	...
	level._so_deadquotes = [];
	level._so_deadquotes[ 0 ] = quote;
	level._so_deadquotes_chance = 1.0;
...
so_include_deadquote_array( quotes )
{
	...
	if ( !isdefined( level._so_deadquotes ) )
		level._so_deadquotes = [];
	level._so_deadquotes = array_merge( level._so_deadquotes , quotes );
}
...
so_hud_pulse_create( new_value )
{
	...
	if ( isdefined( self.pulse_sound ) )
		level._player PlaySound( self.pulse_sound );
...
info_hud_decrement_timer( time )
{
	...
	level._so_challenge_time_left -= time;

	if ( level._so_challenge_time_left < 0 )
	{
		level._so_challenge_time_left = 0.01;
	}
	...
	foreach ( player in level._players )
	{
		player.hud_so_timer_time SetTenthsTimer( level._so_challenge_time_left );
...
is_best_time( time_start, time_current, time_frac )
{
	if ( !isdefined( time_start ) )
	{
		if ( isdefined( level._challenge_start_time ) )
			time_start = level._challenge_start_time;
		else
...
is_poor_time( time_start, time_current, time_frac )
{
	...
	m_seconds = ( time_current - time_start );
	m_time_limit = ( level._challenge_time_limit * 1000 );
...
{
	// Check for best time.
	if ( is_best_time( level._challenge_start_time, level._challenge_end_time ) )
	{
...
so_dialog_mission_failed( sound_alias )
{
	...
	if ( isdefined( level._failed_dialog_played ) && level._failed_dialog_played )
		return;

	level._failed_dialog_played = true;
	so_dialog_play( sound_alias, 0.5, true );
}

so_dialog_mission_failed_generic()
{
	if ( ( level._gameskill <= 2 ) || cointoss() )
...
{
	if ( !isdefined( level._civilian_warning_time ) )
	{
		level._civilian_warning_time = gettime();
		if ( !isdefined( level._civilian_warning_throttle ) )
			level._civilian_warning_throttle = 5000;
	}
	else
	{
		if ( ( gettime() - level._civilian_warning_time ) < level._civilian_warning_throttle )
			return;
	}
	
	wait_time = 0.5;
	level._civilian_warning_time = gettime() + ( wait_time * 1000 );
...
so_dialog_progress_update( current_value, current_goal )
{
	...
	if ( !isdefined( level._so_progress_goal_status ) )
		level._so_progress_goal_status = "none";
	
	time_frac = undefined;
	switch ( level._so_progress_goal_status )
	{
	...
	{
		case "none":
			level._so_progress_goal_status = "3quarter";		
			time_dialog = "so_tf_1_progress_3quarter";
			break;
		case "3quarter":
			level._so_progress_goal_status = "half";		
			time_dialog = "so_tf_1_progress_half";
			break;
		case "half":
			level._so_progress_goal_status = "quarter";	
...
so_dialog_progress_update_time_quality( time_frac )
{
	...
	if ( isdefined( level._challenge_time_limit ) )
	{
		if ( is_poor_time( level._challenge_start_time, gettime(), time_frac ) )
		{
	...
	if ( is_best_time( level._challenge_start_time, gettime(), time_frac ) )
...
so_dialog_counter_update( current_count, current_goal, countdown_divide )
{
	...
	if ( !isdefined( level._so_counter_dialog_time ) )
		level._so_counter_dialog_time = 0;
	if ( gettime() < level._so_counter_dialog_time )
		return;
	...
	if ( adjusted_count > 5 )
	{
		if ( !isdefined( level._challenge_progress_manual_update ) || !level._challenge_progress_manual_update )
		{
			thread so_dialog_progress_update( current_count, current_goal );
			level._so_counter_dialog_time = gettime() + 800;
		}
		return;
	}
