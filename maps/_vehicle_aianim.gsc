...
guy_enter_cleanup_on_death( guy )
{
	guy endon( "vehicle_entered" );
	
	guy waittill( "death" );
	
	//do cleanup 
	if ( self.attachedguys[ self.attachedguys.size - 1 ] == guy )
	{
		array_remove_index( self.attachedguys, self.attachedguys.size - 1 );
	}
	
	if( IsDefined( guy.vehicle_position) )
	{
		self.usedPositions[ guy.vehicle_position ] = false;
	}
	
	self.delayer = 0;
	
	if ( self.riders[ self.riders - 1 ] == guy )
	{
		array_remove_index( self.riders, self.riders - 1 );
	}
}
...
{
	guy endon( "death" );
	guy guy_enter_saveparams();
	self thread guy_enter_cleanup_on_death( guy );
	...
	vehicleanim = level._vehicle_aianims[ type ];
	maxpos = level._vehicle_aianims[ type ].size;
...
//		thread guy_vehicle_death( guy );
	...
	if ( IsAI( guy ) )
	{
		if ( !isDefined( guy.dont_teleport_on_enter_vehicle ) )
		{
	...
	// let the vehicle know that the gunner is dead
	if ( isdefined( vehicleanim[pos].mgturret ) )
	{
		thread gunnerdead( guy );
	}

	self notify( "guy_entered", guy, pos );
	guy notify( "vehicle_entered" );
...
//TagCC<NOTE>: Not sure if this does everything we need it to.
guy_exit( guy )
{
...
handle_attached_guys()
{
	...
	self.attachedguys = [];
	if ( !( IsDefined( level._vehicle_aianims ) && IsDefined( level._vehicle_aianims[ type ] ) ) )
		return;
...
guy_deathimate_me( guy, animpos )
{
	...
	[[ level._global_kill_func ]]( "MOD_RIFLE_BULLET", "torso_upper", origin );
...
handle_detached_guys_check()
{
	...
	AssertMsg( "script sent too many ai to vehicle( max is: " + level._vehicle_aianims[ self.vehicletype ].size + " )" );
}
...
vehicle_hasavailablespots()
{
	...
	if ( level._vehicle_aianims[ self.vehicletype ].size - self.runningtovehicle.size )
		return true;
	else
		return false;
}
...
guy_runtovehicle( guy, vehicle, bGoddriver, group )
{
	...
	vehicleanim = level._vehicle_aianims[ vehicle.vehicletype ];
...
gunnerdead( guy )
{
	...
	// End all other instances of this script on this vehicle
	self notify( "gunner_dead_start" );
	self endon( "gunner_dead_start" );

	// Waittill dead and notify gunner_dead to the vehicle
	guy waittill( "death" );
...
{
	return level._vehicle_aianims[ vehicle.vehicletype ][ pos ];
}
...
setup_aianimthreads()
{
	if ( !isdefined( level._vehicle_aianimthread ) )
		level._vehicle_aianimthread = [];

	if ( !isdefined( level._vehicle_aianimcheck ) )
		level._vehicle_aianimcheck = [];

	level._vehicle_aianimthread[ "idle" ] = ::guy_idle;
	level._vehicle_aianimthread[ "duck" ] = ::guy_duck;

	level._vehicle_aianimthread[ "duck_once" ] = ::guy_duck_once;
	level._vehicle_aianimcheck[ "duck_once" ] = ::guy_duck_once_check;

	level._vehicle_aianimthread[ "weave" ] = ::guy_weave;
	level._vehicle_aianimcheck[ "weave" ] = ::guy_weave_check;

	level._vehicle_aianimthread[ "stand" ] = ::guy_stand;

	level._vehicle_aianimthread[ "turn_right" ] = ::guy_turn_right;
	level._vehicle_aianimcheck[ "turn_right" ] = ::guy_turn_right_check;

	level._vehicle_aianimthread[ "turn_left" ] = ::guy_turn_left;
	level._vehicle_aianimcheck[ "turn_left" ] = ::guy_turn_right_check;

	level._vehicle_aianimthread[ "turn_hardright" ] = ::guy_turn_hardright;

	level._vehicle_aianimthread[ "turn_hardleft" ] = ::guy_turn_hardleft;
	level._vehicle_aianimthread[ "turret_fire" ] = ::guy_turret_fire;
	level._vehicle_aianimthread[ "turret_turnleft" ] = ::guy_turret_turnleft;
	level._vehicle_aianimthread[ "turret_turnright" ] = ::guy_turret_turnright;
	level._vehicle_aianimthread[ "unload" ] = ::guy_unload;

	level._vehicle_aianimthread[ "pre_unload" ] = ::guy_pre_unload;
	level._vehicle_aianimcheck[ "pre_unload" ] = ::guy_pre_unload_check;

	level._vehicle_aianimthread[ "idle_alert" ] = ::guy_idle_alert;
	level._vehicle_aianimcheck[ "idle_alert" ] = ::guy_idle_alert_check;

	level._vehicle_aianimthread[ "idle_alert_to_casual" ] = ::guy_idle_alert_to_casual;
	level._vehicle_aianimcheck[ "idle_alert_to_casual" ] = ::guy_idle_alert_to_casual_check;

	level._vehicle_aianimthread[ "reaction" ] = ::guy_turret_turnright;
}
...
{
	animpos = anim_pos( self, pos );
	vehicleanim = level._vehicle_aianims[ self.vehicletype ];
...
get_unload_group()
{
	...
	{
		unloadgroups = level._vehicle_unloadgroups[ self.vehicletype ][ self.unload_group ];
	}
...
check_unloadgroup( pos, unload_group )
{
	...
	type = self.vehicletype;
	if ( !isdefined( level._vehicle_unloadgroups[ type ] ) )
		return true;// just unloads everybody

	if ( !isdefined( level._vehicle_unloadgroups[ type ][ unload_group ] ) )
	{
	...
	group = level._vehicle_unloadgroups[ type ][ unload_group ];
...
getoutrig_model( animpos, model, tag, animation, bIdletillunload )
{
	...
	if ( bIdletillunload )
	{
		thread getoutrig_model_idle( model, tag, level._vehicle_attachedmodels[ type ][ animpos.fastroperig ].idleanim );
...
getout_rigspawn( animatemodel, pos, bIdletillunload )
{
	...
	origin =  animatemodel GetTagOrigin( level._vehicle_attachedmodels[ type ][ animpos.fastroperig ].tag );
	angles =  animatemodel GetTagAngles( level._vehicle_attachedmodels[ type ][ animpos.fastroperig ].tag );
	...
	getoutrig_model SetModel( level._vehicle_attachedmodels[ type ][ animpos.fastroperig ].model );
...
// 			getoutrig_model UseAnimTree( level.vehicle_attachedmodels[ type ][ animpos.fastroperig ].animtree );
	...
	getoutrig_model LinkTo( animatemodel, level._vehicle_attachedmodels[ type ][ animpos.fastroperig ].tag, ( 0, 0, 0 ), ( 0, 0, 0 ) );
	thread getoutrig_model( animpos, getoutrig_model, level._vehicle_attachedmodels[ type ][ animpos.fastroperig ].tag, level._vehicle_attachedmodels[ type ][ animpos.fastroperig ].dropanim, bIdletillunload );
...
guy_unload( guy, pos )
{
	...
	if ( IsDefined( animpos.unload_callback ))
	{
		guy thread [[ animpos.unload_callback ]]( animpos );
	}
	...
	// Allow death if exit_allow_death is set OR block_ragdoll_fail is set
	if ( guy can_ragdoll_fall() && ( !isdefined( animpos.exit_allow_death ) || !animpos.exit_allow_death ) )
	{
		// Nobody should die during the transition.
		guy.allowdeath = false;
	}

	if ( isdefined( animpos.exit_death_ragdoll_immediate ) && animpos.exit_death_ragdoll_immediate )
	{
		guy.ragdoll_immediate = true;
	}
	...
	if ( !bNoanimUnload )
	{
		...
		if ( IsDefined( guy.playerpiggyback ) && IsDefined( animpos.player_getout_sound_loop ) )
			level._player thread play_loop_sound_on_entity( animpos.player_getout_sound_loop );
		...
		else
		{
			...
			animontag( guy, tag, animation, undefined, undefined, "vehicle_getout" );
		}

		// end all the loop sounds
		if ( IsDefined( guy.playerpiggyback ) && IsDefined( animpos.player_getout_sound_loop ) )
			level._player thread stop_loop_sound_on_entity( animpos.player_getout_sound_loop );
		...
		if ( IsDefined( guy.playerpiggyback ) && IsDefined( animpos.player_getout_sound_end ) )
			level._player thread play_sound_on_entity( animpos.player_getout_sound_end );
	}
	else
	{
		if ( !isai( guy ) )
...
guy_enter_saveparams()
{
	self.vehicle_saved_params 					= spawnstruct();
	if ( isdefined( self.noRagdoll ) )
	{
		self.vehicle_saved_params.noRagdoll 	= self.noRagdoll;
	}
	else
	{
		self.vehicle_saved_params.noRagdoll 	= 0;
	}

	if ( isAI( self ) )
	{
		self.vehicle_saved_params.disableLongDeath 	= self.a.disableLongDeath;
	}
}

guy_unload_restoreparams()
{
	if ( !isdefined( self.vehicle_saved_params ) )
	{
		return;
	}
	...
	if ( isAI( self ) )
	{
		self.a.disableLongDeath 					= self.vehicle_saved_params.disableLongDeath;
	}
	
	self.noRagdoll 								= self.vehicle_saved_params.noRagdoll;
	self.vehicle_saved_params 					= undefined;
}
...
{
	if ( IsDefined( self.troops_go_down_with_vehicle ))
	{
		if ( self.troops_go_down_with_vehicle )
		{
			return;
		}
	}
...
{
	animpos = level._vehicle_aianims[ self.vehicletype ][ pos ];
...
can_ragdoll_fall()
{
	if ( IsDefined( self.block_ragdoll_fall ) && self.block_ragdoll_fall == true )
	{
		return false;
	}
	return true;
...
guy_unlink_on_death( guy )
{
	...
	if ( IsDefined( guy ) && guy can_ragdoll_fall())
	{
		guy Unlink();
	}
}
...
guy_blowup( guy )
{
	...
	[[ level._global_kill_func ]]( "MOD_RIFLE_BULLET", "torso_upper", guy.origin );
...
is_position_in_group( vehicle, pos, group )
{
	...
	Assert( IsDefined( level._vehicle_unloadgroups[ vehicle.vehicletype ][ group ] ) );
	vehicles_group = level._vehicle_unloadgroups[ vehicle.vehicletype ][ group ];
...
{
	level._vehicle_aianims[ type ][ pos ].vehicle_standattack = animation;
}
...
animate_guys( other )
{
	...
	foreach ( guy in self.riders )
	{
		...
		if ( IsDefined( level._vehicle_aianimcheck[ other ] ) && ! [[ level._vehicle_aianimcheck[ other ] ]]( guy, guy.vehicle_position ) )
			continue;// ignore this if they have a check function and this anim doesn't exist

		if ( IsDefined( level._vehicle_aianimthread[ other ] ) )
		{
				guy notify( "newanim" );
				guy.queued_anim_threads = [];// sorry que, this animation is more important.
				thread [[ level._vehicle_aianimthread[ other ] ]]( guy, guy.vehicle_position );
